...
from datagod.ml.anomaly_detector import AnomalyConfig, AnomalyDetector, AnomalyRule
from datagod.ml.anomaly_detector import AnomalySeverity as DetectorSeverity
from datagod.ml.anomaly_detector import get_anomaly_detector
...
try:
    from api.src.api_v2 import RoleChecker, User, get_current_active_user
...
async def detect_anomalies(
    ...
    try:
        ...
        from datagod.models import Jurisdiction, Record
...
async def list_anomalies(
    ...
    if anomaly_type:
        all_anomalies = [
            a for a in all_anomalies if a.anomaly_type.value == anomaly_type
        ]

    # Calculate stats
    unresolved = [
        a for a in detector.get_all_anomalies(include_resolved=True) if not a.resolved
    ]
    by_severity = {}
    by_type = {}
...
async def get_anomaly(
    anomaly_id: str, current_user: User = Depends(get_current_active_user)
):
    """
...
async def get_anomaly_stats(current_user: User = Depends(get_current_active_user)):
    """
...
async def list_rules(current_user: User = Depends(get_current_active_user)):
    ...
    return AnomalyRuleListResponse(
        rules=rule_responses, total=len(rules), enabled_count=enabled_count
    )
...
async def create_rule(
    rule_data: AnomalyRuleCreate, current_user: User = Depends(get_current_active_user)
):
    """
...
async def delete_rule(
    rule_id: str, current_user: User = Depends(get_current_active_user)
):
    """
...
async def update_config(
    ...
    if update_data.isolation_forest_contamination is not None:
        config.isolation_forest_contamination = (
            update_data.isolation_forest_contamination
        )
