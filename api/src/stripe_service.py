...
STRIPE_PRICE_IDS = {
    ...
    "enterprise": os.getenv(
        "STRIPE_PRICE_ID_ENTERPRISE", "price_enterprise_placeholder"
    ),
}
...
class StripeService:
    ...
    def __init__(self):
        self.stripe_available = STRIPE_AVAILABLE and bool(
    ...
    def upgrade_subscription(
        self, subscription_id: str, new_price_id: str, prorate: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            updated = stripe.Subscription.modify(
                ...
                idempotency_key=self._generate_idempotency_key(
                    "upgrade", subscription_id, new_price_id
                ),
            )
            ...
            return [
                {
                    ...
                    "period_start": (
                        datetime.fromtimestamp(inv.period_start).isoformat()
                        if inv.period_start
                        else None
                    ),
                    "period_end": (
                        datetime.fromtimestamp(inv.period_end).isoformat()
                        if inv.period_end
                        else None
                    ),
    ...
    def create_usage_record(
        self, subscription_item_id: str, quantity: int, action: str = "increment"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            record = stripe.SubscriptionItem.create_usage_record(
                ...
                idempotency_key=self._generate_idempotency_key(
                    "usage", subscription_item_id, quantity
                ),
            )
    ...
    def apply_coupon(self, customer_id: str, coupon_code: str) -> Dict[str, Any]:
        """
