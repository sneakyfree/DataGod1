...
class QualityConfig:
    ...
    required_fields: List[str] = field(
        default_factory=lambda: ["id", "jurisdiction_id", "record_type", "created_at"]
    )
...
class DataQualityMonitor:
    ...
    def generate_report(
        self, data: pd.DataFrame, table_name: str = "records"
    ) -> QualityReport:
        """
        ...
        metrics[QualityDimension.COMPLETENESS] = self.calculate_completeness(
            data, table_name
        )
        ...
        metrics[QualityDimension.CONSISTENCY] = self.calculate_consistency(
            data, table_name
        )
        issues.extend(self._get_consistency_issues(data, table_name))

        metrics[QualityDimension.TIMELINESS] = self.calculate_timeliness(
            data, table_name
        )
        ...
        metrics[QualityDimension.UNIQUENESS] = self.calculate_uniqueness(
            data, table_name
        )
        ...
        overall_score = sum(
            metrics[dim].score * weights.get(dim, 0.1) for dim in metrics
        )
        ...
        for field, pct in field_completeness.items():
            if pct < 90:
                recommendations.append(
                    f"Field '{field}' has only {pct}% completeness - review data ingestion"
                )
    ...
    def calculate_accuracy(
        self, data: pd.DataFrame, table_name: str = "records", sample_size: int = 100
    ) -> QualityMetric:
        """
        ...
        date_cols = [
            ...
            if "date" in c.lower() or c in ["created_at", "updated_at"]
        ]
            ...
            if col in sample.columns:
                try:
                    ...
                    valid_dates = (
                        ...
                        & (dates <= pd.Timestamp.now() + pd.Timedelta(days=365))
                    )
    ...
    def calculate_consistency(
        ...
        overall_consistency = (
            np.mean(consistency_scores) if consistency_scores else 100.0
        )
    ...
    def calculate_timeliness(
        ...
        if age <= sla:
            ...
            timeliness = max(
                0, 100 - (age.days / self.config.freshness_threshold_days * 50)
            )
        else:
    ...
    def calculate_validity(
        ...
        # ZIP code validation
        zip_cols = [
            c for c in data.columns if "zip" in c.lower() or "postal" in c.lower()
        ]
        for col in zip_cols:
        ...
        for field in required_fields:
            ...
            if null_pct > 5:
                severity = (
                    "critical"
                    if null_pct > 20
                    else "high" if null_pct > 10 else "medium"
                )
                issues.append(
                    QualityIssue(
                        issue_id=self._generate_id(
                            f"completeness_{table_name}_{field}"
                        ),
                        dimension=QualityDimension.COMPLETENESS,
                        severity=severity,
                        table=table_name,
    ...
    def _generate_recommendations(
        self, metrics: Dict[QualityDimension, QualityMetric], issues: List[QualityIssue]
    ) -> List[str]:
        ...
        if high_issues:
            recommendations.append(
                f"Review and fix {len(high_issues)} high-priority quality issues"
            )
