...
class MortgageDataGatheringNeuralNetwork:
    ...
    def _extract_from_court_records(self, data: str) -> List[MortgageDataPoint]:
        ...
        borrower_names = self._extract_by_pattern(
            data, "borrower_name", borrower_patterns
        )
        ...
        max_count = max(
            len(loan_amounts), len(borrower_names), len(lender_names), len(loan_dates)
        )
    ...
    def _extract_by_pattern(
        self, text: str, pattern_type: str, patterns: List[str]
    ) -> List[str]:
            ...
            try:
                ...
                for i, vector in enumerate(vectors):
                    ...
                    similar_indices = [
                        ...
                        if sim > self.similarity_threshold and j != i
                    ]

                    # If we found similar points, potentially merge or enhance
                    if similar_indices:
                        # For now, just enhance the quality score
                        data_points[i].quality_score = min(
                            100.0,
                            (
                                data_points[i].quality_score + 10
                                if data_points[i].quality_score
                                else 10
                            ),
                        )
    ...
    def get_data_insights(self, data_points: List[MortgageDataPoint]) -> Dict[str, Any]:
        ...
        total_loan_amount = sum(
            dp.loan_amount for dp in data_points if dp.loan_amount > 0
        )
        total_interest_rate = sum(
            dp.interest_rate for dp in data_points if dp.interest_rate > 0
        )
        total_loan_term = sum(dp.loan_term for dp in data_points if dp.loan_term > 0)

        insights["average_loan_amount"] = (
            total_loan_amount / len(data_points) if data_points else 0.0
        )
        insights["average_interest_rate"] = (
            total_interest_rate / len(data_points) if data_points else 0.0
        )
        insights["average_loan_term"] = (
            total_loan_term / len(data_points) if data_points else 0.0
        )
        ...
        insights["loan_type_distribution"] = {
            lt: loan_types.count(lt) for lt in set(loan_types)
        }
        insights["status_distribution"] = {s: statuses.count(s) for s in set(statuses)}

        # Property value range
        property_values = [
            dp.property_value for dp in data_points if dp.property_value > 0
        ]
        if property_values:
            insights["property_value_range"] = [
                min(property_values),
                max(property_values),
            ]
