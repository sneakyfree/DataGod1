...
class AnalyticsEngine:
    ...
    def distribution_analysis(self, data: pd.DataFrame, column: str) -> DistributionReport:
        """
        ...
        for i in range(len(columns_list)):
            for j in range(i + 1, len(columns_list)):
                ...
                if not np.isnan(corr) and abs(corr) > 0.3:  # Significant threshold
                    ...
                    if len(valid_data) >= 3:
                        if method == 'pearson':
                            _, p_value = scipy_stats.pearsonr(valid_data[col1], valid_data[col2])
                        elif method == 'spearman':
                            _, p_value = scipy_stats.spearmanr(valid_data[col1], valid_data[col2])
                        else:
                            _, p_value = scipy_stats.kendalltau(valid_data[col1], valid_data[col2])
    ...
    def aggregation_analysis(
        ...
        if not group_by:
            return AggregationReport(
                group_by=[], metrics=[], results=[], summary={}
            )
        ...
        if date_column and date_column in data.columns:
            ...
            if len(df) > 0:
                ...
                insights.append(Insight(
                    ...
                    metadata={"earliest": earliest.isoformat(), "latest": latest.isoformat()},
                ))
        ...
        if len(numeric_cols) >= 2:
            ...
            for pair in corr.significant_pairs[:3]:
                if abs(pair["correlation"]) > 0.5:
                    insights.append(Insight(
                        id=self._generate_id(f"corr_{pair['column1']}_{pair['column2']}"),
    ...
    def get_quick_stats(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "null_percentage": round((data.isnull().sum().sum() / data.size) * 100, 2) if data.size > 0 else 0,
        }
