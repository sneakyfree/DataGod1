...
class MortgageDataGatheringNeuralNetwork(nn.Module):
    ...
    def __init__(self, input_size: int = 1000, hidden_size: int = 512, output_size: int = 10):
    ...
    def _extract_from_property_records(self, data: str) -> List[MortgageDataPoint]:
        ...
        try:
            ...
            if len(vectorized_data) > 0:
                ...
                confidence_score = float(output[0][0].item()) * 100  # Convert to percentage
            else:
            ...
            if matches:
                for match in matches:
                    data_point = MortgageDataPoint(
                        ...
                        lender_name="Unknown",
                        loan_amount=float(match.replace('$', '').replace(',', '')) if match else 0.0,
        ...
        if loan_amounts:
            for amount in loan_amounts:
                data_point = MortgageDataPoint(
                    ...
                    borrower_name="Unknown",
                    lender_name="Unknown",
                    loan_amount=float(amount.replace('$', '').replace(',', '')) if amount else 0.0,
                    loan_type="Unknown",
                    interest_rate=0.0,
                    loan_term=30,
                    loan_date=datetime.now().strftime("%Y-%m-%d"),
