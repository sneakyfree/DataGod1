...
class PredictiveEngine:
    ...
    def forecast_volume(
        ...
        try:
            ...
            if method == ForecastMethod.MOVING_AVERAGE:
                predictions, metrics = self._forecast_moving_average(
                    df, date_column, value_column, horizon
                )
            elif method == ForecastMethod.EXPONENTIAL_SMOOTHING:
                predictions, metrics = self._forecast_exponential_smoothing(
                    df, date_column, value_column, horizon
                )
            elif method == ForecastMethod.LINEAR_REGRESSION:
                predictions, metrics = self._forecast_linear_regression(
                    df, date_column, value_column, horizon
                )
    ...
    def detect_patterns(
        self, data: pd.DataFrame, date_column: str = "date", value_column: str = "count"
    ) -> List[Pattern]:
        """
        ...
        for j in jurisdictions:
            ...
            if last_updated:
                ...
                if days_since_update > update_frequency * 1.5:
                    probability = min(
                        0.5 + (days_since_update - update_frequency) * 0.05, 0.95
                    )
    ...
    def get_insights(
        ...
        try:
            ...
            avg = values.mean()
            insights.append(
                f"Total volume: {total:,.0f} records with a daily average of {avg:,.1f}"
            )
            ...
            if len(values) >= 14:
                ...
                if abs(change_pct) > 10:
                    ...
                    insights.append(
                        f"Volume has {direction} by {abs(change_pct):.1f}% over the analysis period"
                    )
            ...
            insights.append(
                f"Highest single-day volume of {max_val:,.0f} records occurred on {max_date.strftime('%B %d, %Y')}"
            )
            ...
            if cv > 50:
                insights.append(
                    f"High variability detected (CV: {cv:.1f}%) - data collection may be inconsistent"
                )
            elif cv < 20:
                insights.append(
                    f"Volume is stable with low variability (CV: {cv:.1f}%)"
                )
            ...
            if len(values) >= 7:
                ...
                if abs(recent_change) > 15:
                    ...
                    insights.append(
                        f"Recent 7-day average is {abs(recent_change):.1f}% {direction} the overall average"
                    )
    ...
    def _forecast_moving_average(
        self, df: pd.DataFrame, date_col: str, value_col: str, horizon: int
        ...
        metrics = {
            ...
            "mape": (
                float(np.abs((values - ma) / values).mean() * 100)
                if values.mean() > 0
                else 0
            ),
            "window_size": window,
    ...
    def _detect_trend(
        ...
        if r_squared > 0.3:  # Significant trend
            if slope > 0:
                ...
                description = (
                    f"Upward trend detected with {slope:.1f} increase per period"
                )
            else:
                pattern_type = PatternType.TREND_DOWN
                description = (
                    f"Downward trend detected with {abs(slope):.1f} decrease per period"
                )
    ...
    def _generate_cache_key(
        self, data: pd.DataFrame, method: ForecastMethod, horizon: int
    ) -> str:
        """Generate a cache key for the forecast."""
        data_hash = hashlib.md5(
            str(data.values.tobytes()).encode(), usedforsecurity=False
        ).hexdigest()[:8]
