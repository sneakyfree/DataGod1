...
class ModelRegistry:
    ...
    def promote_to_production(
        self, version_id: str, traffic_percentage: float = 100.0
    ) -> ModelVersion:
        """
        ...
        for v in self._versions.values():
            if (
                v.model_name == version.model_name
                and v.status == ModelStatus.PRODUCTION
            ):
        ...
        self._save_metadata()
        logger.info(
            "Promoted %s to production with %s%% traffic",
            version_id,
            traffic_percentage,
        )
    ...
    def rollback(self, model_name: str, target_version: str = None) -> ModelVersion:
        ...
        if len(model_versions) < 2:
            raise ValueError(
                f"No previous version available for rollback: {model_name}"
            )

        # Find target version
        if target_version:
            target = next(
                (v for v in model_versions if v.version_number == target_version), None
            )
            if not target:
    ...
    def start_ab_test(
        ...
        test_id = hashlib.md5(
            f"{model_name}_{test_name}_{datetime.utcnow().isoformat()}".encode(),
        ...
        self._versions[treatment_version_id].traffic_percentage = (
            100 - control_traffic_pct
        )
    ...
    def complete_ab_test(self, test_id: str, winner: str = None) -> ABTest:
        ...
        if winner:
            winner_id = (
                ab_test.control_version_id
                if winner == "control"
                else ab_test.treatment_version_id
            )
            loser_id = (
                ab_test.treatment_version_id
                if winner == "control"
                else ab_test.control_version_id
            )
    ...
    def list_versions(
        self, model_name: str = None, status: ModelStatus = None, limit: int = 100
    ) -> List[ModelVersion]:
    ...
    def _generate_version_id(self, model_name: str, version_number: str) -> str:
        ...
        return hashlib.md5(
            f"{model_name}_{version_number}".encode(), usedforsecurity=False
        ).hexdigest()[:16]
    ...
    def _load_metadata(self) -> None:
        ...
        try:
            ...
            for test_data in metadata.get("ab_tests", {}).values():
                ...
                test_data["end_date"] = (
                    datetime.fromisoformat(test_data["end_date"])
