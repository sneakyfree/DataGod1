...
class PropertySchema(BaseSchema):
    ...
    def __init__(self):
        fields = [
            ...
            FieldSchema(
                "address", FieldType.STRING, required=True, min_length=5, max_length=200
            ),
            FieldSchema(
                "city", FieldType.STRING, required=True, min_length=1, max_length=100
            ),
            FieldSchema(
                "state",
                FieldType.STRING,
            ...
            FieldSchema(
                "zip_code", FieldType.STRING, required=True, pattern=r"^\d{5}(-\d{4})?$"
            ),
            FieldSchema("county", FieldType.STRING, required=False, max_length=100),
            FieldSchema(
                "property_type",
                FieldType.STRING,
                required=False,
            ...
            FieldSchema(
                "bedrooms", FieldType.INTEGER, required=False, min_value=0, max_value=50
            ),
            FieldSchema(
                "bathrooms", FieldType.FLOAT, required=False, min_value=0, max_value=50
            ),
            FieldSchema(
                "square_feet",
                FieldType.FLOAT,
            ...
            FieldSchema("lot_size", FieldType.FLOAT, required=False, min_value=0),
            FieldSchema(
                "year_built",
                FieldType.INTEGER,
                required=False,
            ...
            FieldSchema("assessed_value", FieldType.FLOAT, required=False, min_value=0),
            FieldSchema("market_value", FieldType.FLOAT, required=False, min_value=0),
            FieldSchema("owner_name", FieldType.STRING, required=False, max_length=200),
            FieldSchema("last_sale_date", FieldType.DATE, required=False),
            FieldSchema(
                "last_sale_price", FieldType.FLOAT, required=False, min_value=0
            ),
            FieldSchema(
                "latitude", FieldType.FLOAT, required=False, min_value=-90, max_value=90
            ),
            FieldSchema(
                "longitude",
                FieldType.FLOAT,
...
class DeedSchema(BaseSchema):
    ...
    def __init__(self):
        fields = [
            ...
            FieldSchema("grantor", FieldType.STRING, required=False, max_length=500),
            FieldSchema("grantee", FieldType.STRING, required=False, max_length=500),
            FieldSchema("consideration", FieldType.FLOAT, required=False, min_value=0),
            FieldSchema("parcel_id", FieldType.STRING, required=False, max_length=50),
            FieldSchema(
                "legal_description", FieldType.STRING, required=False, max_length=2000
            ),
            FieldSchema("book", FieldType.STRING, required=False, max_length=20),
            FieldSchema("page", FieldType.STRING, required=False, max_length=20),
            FieldSchema("county", FieldType.STRING, required=True, max_length=100),
            FieldSchema(
                "state", FieldType.STRING, required=True, min_length=2, max_length=2
            ),
        ]
...
class CourtCaseSchema(BaseSchema):
    ...
    def __init__(self):
        fields = [
            ...
            FieldSchema("court_name", FieldType.STRING, required=True, max_length=200),
            ...
            FieldSchema("case_title", FieldType.STRING, required=False, max_length=500),
            FieldSchema("plaintiff", FieldType.STRING, required=False, max_length=500),
            FieldSchema("defendant", FieldType.STRING, required=False, max_length=500),
            FieldSchema("judge", FieldType.STRING, required=False, max_length=200),
            FieldSchema("amount_claimed", FieldType.FLOAT, required=False, min_value=0),
            FieldSchema(
                "judgment_amount", FieldType.FLOAT, required=False, min_value=0
            ),
...
class BusinessEntitySchema(BaseSchema):
    ...
    def __init__(self):
        fields = [
            ...
            FieldSchema(
                "ein", FieldType.STRING, required=False, pattern=r"^\d{2}-\d{7}$"
            ),
            FieldSchema(
                "registered_agent", FieldType.STRING, required=False, max_length=300
            ),
            FieldSchema(
                "registered_address", FieldType.STRING, required=False, max_length=500
            ),
            FieldSchema(
                "principal_address", FieldType.STRING, required=False, max_length=500
            ),
        ]
...
class PersonSchema(BaseSchema):
    ...
    def __init__(self):
        fields = [
            ...
            FieldSchema(
                "middle_name", FieldType.STRING, required=False, max_length=100
            ),
            FieldSchema(
                "suffix",
                FieldType.STRING,
            ...
            FieldSchema(
                "ssn_last4", FieldType.STRING, required=False, pattern=r"^\d{4}$"
            ),
            FieldSchema("address", FieldType.STRING, required=False, max_length=500),
            FieldSchema("city", FieldType.STRING, required=False, max_length=100),
            FieldSchema(
                "state", FieldType.STRING, required=False, min_length=2, max_length=2
            ),
            FieldSchema(
                "zip_code",
                FieldType.STRING,
            ...
            FieldSchema(
                "phone", FieldType.STRING, required=False, pattern=r"^\+?1?\d{10,14}$"
            ),
            FieldSchema(
                "email",
                FieldType.STRING,
...
class SchemaValidator:
    ...
    def validate(
        self, record: Dict[str, Any], schema: Union[str, BaseSchema]
    ) -> ValidationResult:
        """
    ...
    def _validate_field(
        self, result: ValidationResult, field_name: str, value: Any, schema: FieldSchema
    ):
        ...
        if schema.allowed_values:
            if (
                value not in schema.allowed_values
                and str(value).upper() not in schema.allowed_values
            ):
                result.add_error(
                    field=field_name,
            ...
            try:
                if not schema.custom_validator(value):
                    result.add_error(
                        ...
                        or f"'{field_name}' failed custom validation",
                        actual=value,
                    )
    ...
    def validate_batch(
        self, records: List[Dict[str, Any]], schema: Union[str, BaseSchema]
