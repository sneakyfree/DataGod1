...
class AuditLogger:
    ...
    def query(
        ...
        logger.debug(
            "Audit query: action=%s, user=%s, resource=%s/%s",
            action,
            user_id,
            resource_type,
            resource_id,
    ...
    def export(
        ...
        **filters,
        ...
        filename = f"/tmp/audit_export_{timestamp}.{format}"  # nosec B108 - non-sensitive audit temp path
    ...
    def _setup_file_logging(self):
        ...
        if log_dir and not os.path.exists(log_dir):
            try:
                ...
                self.log_file = f"/tmp/datagod_audit_{datetime.utcnow().strftime('%Y%m%d')}.log"  # nosec B108 - non-sensitive audit temp path
    ...
    def _get_default_severity(
        self, action: AuditAction, success: bool
    ) -> AuditSeverity:
    ...
    def decorator(func):
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                audit_logger.log(
                    action=action, resource_type=resource_type, success=True
                )
                return result
            except Exception as e:
                audit_logger.log(
