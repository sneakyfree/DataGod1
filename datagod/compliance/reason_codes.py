...
class ReasonCodeEngine:
    ...
    def _initialize_catalog(self) -> Dict[str, ReasonCode]:
        ...
        fcra_codes = [
            ReasonCode("FCRA-01", ReasonCodeStandard.FCRA, "Derogatory Public Record",
                       "Public record information indicates adverse legal action",
                       "15 U.S.C. § 1681(a)", ReasonCodeSeverity.HIGH,
                       "legal", "Review and dispute inaccurate public records",
                       ["judgment_lien", "bankruptcy"]),
            ReasonCode("FCRA-02", ReasonCodeStandard.FCRA, "Collection Account",
                       "Outstanding collection account(s) on record",
                       "15 U.S.C. § 1681(a)", ReasonCodeSeverity.HIGH,
                       "financial", "Settle or dispute collection accounts",
                       ["collection_action"]),
            ReasonCode("FCRA-03", ReasonCodeStandard.FCRA, "Tax Lien Filed",
                       "Federal, state, or local tax lien filed against property",
                       "15 U.S.C. § 1681(a)", ReasonCodeSeverity.CRITICAL,
                       "tax", "Pay outstanding tax liability or negotiate payment plan",
                       ["tax_lien", "federal_tax_lien", "state_tax_lien"]),
            ReasonCode("FCRA-04", ReasonCodeStandard.FCRA, "Judgment Entered",
                       "Civil judgment entered against property owner or entity",
                       "15 U.S.C. § 1681(a)", ReasonCodeSeverity.HIGH,
                       "legal", "Satisfy judgment or negotiate settlement",
                       ["judgment_lien", "civil_judgment"]),
        ]

        # --- ECOA-Adjacent Reason Codes ---
        ecoa_codes = [
            ReasonCode("ECOA-01", ReasonCodeStandard.ECOA, "Insufficient Collateral",
                       "Property value insufficient relative to proposed transaction",
                       "12 CFR § 1002.9", ReasonCodeSeverity.MEDIUM,
                       "property", "Obtain updated appraisal or reduce transaction amount",
                       ["low_value", "insufficient_equity"]),
            ReasonCode("ECOA-02", ReasonCodeStandard.ECOA, "Unclear Title",
                       "Title defects prevent clear conveyance",
                       "12 CFR § 1002.9", ReasonCodeSeverity.HIGH,
                       "title", "Obtain title insurance or resolve title defects",
                       ["title_defect", "chain_of_title", "competing_claim"]),
            ReasonCode("ECOA-03", ReasonCodeStandard.ECOA, "Excessive Existing Liens",
                       "Total existing liens exceed acceptable threshold relative to value",
                       "12 CFR § 1002.9", ReasonCodeSeverity.HIGH,
                       "financial", "Pay down existing liens or negotiate subordination",
                       ["excessive_liens", "over_leveraged"]),
        ]

        # --- DataGod Internal Codes ---
        dg_codes = [
            ReasonCode("DG-001", ReasonCodeStandard.DATAGOD, "Mechanics Lien Active",
                       "Active mechanics lien indicates unpaid construction work",
                       "State Lien Law (varies)", ReasonCodeSeverity.MEDIUM,
                       "lien", "Negotiate with contractor or post bond",
                       ["mechanics_lien"]),
            ReasonCode("DG-002", ReasonCodeStandard.DATAGOD, "HOA Lien Delinquent",
                       "HOA assessment lien for unpaid dues",
                       "CC&Rs / State HOA Statute", ReasonCodeSeverity.MEDIUM,
                       "lien", "Pay outstanding HOA dues and request lien release",
                       ["hoa_lien"]),
            ReasonCode("DG-003", ReasonCodeStandard.DATAGOD, "Environmental Hazard Flag",
                       "Environmental contamination or hazard identified at property",
                       "CERCLA / State ENV Law", ReasonCodeSeverity.CRITICAL,
                       "environmental", "Obtain Phase I/II environmental assessment",
                       ["environmental_hazard", "contamination"]),
            ReasonCode("DG-004", ReasonCodeStandard.DATAGOD, "Regulatory Non-Compliance",
                       "Property or entity fails to meet regulatory requirements",
                       "Varies by jurisdiction", ReasonCodeSeverity.HIGH,
                       "regulatory", "Consult legal counsel for compliance plan",
                       ["code_violation", "permit_issue", "zoning_violation"]),
            ReasonCode("DG-005", ReasonCodeStandard.DATAGOD, "Entity Verification Failed",
                       "Owning entity could not be verified in public records",
                       "State SOS Records", ReasonCodeSeverity.MEDIUM,
                       "entity", "Verify entity registration with Secretary of State",
                       ["entity_not_found", "entity_inactive"]),
            ReasonCode("DG-006", ReasonCodeStandard.DATAGOD, "Data Staleness Warning",
                       "Underlying data is older than acceptable freshness threshold",
                       "DataGod Data Quality Policy", ReasonCodeSeverity.LOW,
                       "data_quality", "Request fresh data pull from source jurisdictions",
                       ["stale_data"]),
            ReasonCode("DG-007", ReasonCodeStandard.DATAGOD, "Vacant Property Risk",
                       "Property appears unoccupied, increasing risk profile",
                       "Local Property Maintenance Code", ReasonCodeSeverity.LOW,
                       "property", "Arrange property inspection and secure premises",
                       ["vacant_property"]),
            ReasonCode("DG-008", ReasonCodeStandard.DATAGOD, "Distressed Property Flag",
                       "Multiple indicators suggest property is in financial distress",
                       "DataGod Risk Engine", ReasonCodeSeverity.HIGH,
                       "risk", "Evaluate acquisition opportunity or risk mitigation",
    ...
    def generate_codes(
        ...
        critical_count = sum(1 for c in codes if c.severity == ReasonCodeSeverity.CRITICAL)
    ...
    def list_codes(
        ...
        codes = [
            rc for key, rc in self._catalog.items()
            if not key.startswith("blocker:") and
            (standard is None or rc.standard == standard)
        ]
        return codes
