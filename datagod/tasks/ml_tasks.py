...
def run_anomaly_detection(self, record_ids: Optional[List[int]] = None, detection_method: str = "statistical"):
    """
    ...
    """
    try:
        logger.info(f"Starting anomaly detection: method={detection_method}, records={len(record_ids) if record_ids else 'all'}")
        ...
        anomaly_count = len([r for r in results if r.get("is_anomaly", False)]) if results else 0

        logger.info(f"Anomaly detection completed: {anomaly_count} anomalies found in {len(results) if results else 0} records")
...
def check_data_quality():
    ...
    try:
        ...
        try:
            # Count total records
            total_records = session.execute("SELECT COUNT(*) FROM records").scalar() or 0
            # Count records with missing fields
            missing_title = session.execute("SELECT COUNT(*) FROM records WHERE title IS NULL OR title = ''").scalar() or 0
            missing_date = session.execute("SELECT COUNT(*) FROM records WHERE date IS NULL").scalar() or 0
            missing_amount = session.execute("SELECT COUNT(*) FROM records WHERE amount IS NULL").scalar() or 0

            completeness = 1.0 - ((missing_title + missing_date + missing_amount) / (total_records * 3)) if total_records > 0 else 0
            ...
            logger.info(f"Data quality check completed: completeness={result['completeness_score']}%")
            return result
        finally:
            session.close()
