...
class MultiViewReportGenerator:
    ...
    def generate(
        ...
        if "liens" in data or "lien_search" in data:
            report["sections"].append(self._generate_lien_section(data, view, template))
        ...
        if "risk_assessment" in data:
            report["sections"].append(self._generate_risk_section(data, view, template))

        # Add summary section
        report["sections"].insert(
            0, self._generate_summary_section(data, view, template)
        )

        # Filter sections if specified
        if include_sections:
            report["sections"] = [
                s for s in report["sections"] if s["id"] in include_sections
            ]
    ...
    def export(
        self, report: Dict[str, Any], format: ExportFormat = ExportFormat.JSON
    ) -> Union[str, bytes]:
        """
    ...
    def _generate_summary_section(
        self, data: Dict[str, Any], view: ReportView, template: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        summary_text = self._format_for_view(key_facts, view, template)

        return {
            "id": "summary",
            "title": "Executive Summary" if view != ReportView.CONSUMER else "Summary",
            "content": {"key_facts": key_facts, "text": summary_text},
            "order": 0,
        }
    ...
    def _generate_lien_section(
        ...
        if view == ReportView.CONSUMER:
            if liens:
                ...
                description = (
                    f"No liens were found in the searched records. {disclaimer}"
                )
        else:
    ...
    def _generate_risk_section(
        ...
        if view == ReportView.CONSUMER:
            ...
            description = risk_descriptions.get(
                category, "Risk level could not be determined."
            )
        else:
    ...
    def _generate_footer(self, view: ReportView) -> Dict[str, Any]:
        ...
        if view == ReportView.AUDIT:
            footer["audit_note"] = (
                "This report includes complete provenance and is suitable for audit purposes."
            )

        return footer

    def _build_audit_metadata(
        self, data: Dict[str, Any], extra_metadata: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _format_for_view(
        self, items: List[str], view: ReportView, template: Dict[str, Any]
    ) -> str:
