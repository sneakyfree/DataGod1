...
from .schemas import (
    AgentTask, AgentOutput, AgentAction, AgentStatus, 
    AgentPriority, ConfidenceLevel, EvidenceRef
)
...
class OrchestratorAgent:
    ...
    SPECIALIST_AGENTS = {
        ...
        "lien_priority": {
            ...
            "priority_keywords": ["lien", "priority", "encumbrance", "judgment", "clear"]
        },
        "risk_assessment": {
            "name": "Risk Assessment Agent",
            "description": "Identifies and scores documented risks",
            "capabilities": ["risk_scoring", "red_flag_detection", "distress_signals"],
            "priority_keywords": ["risk", "red flag", "distress", "foreclosure", "delinquent"]
        },
        "compliance_check": {
    ...
    async def process_query(
        ...
        try:
            ...
            if not guardrail_result.passed:
                synthesis.warnings.extend([v.get('message', '') for v in guardrail_result.violations])
            ...
            # Step 5: Finalize
            root_task.status = AgentStatus.COMPLETED if guardrail_result.passed else AgentStatus.AWAITING_APPROVAL
            
            self._log_action(
                task_id=root_task.task_id,
                action="task_completed",
                outputs={"result": synthesis.result, "confidence": synthesis.confidence}
            )
    ...
    async def _decompose_task(self, task: AgentTask) -> List[AgentTask]:
        ...
        self._log_action(
            ...
            outputs={"subtask_count": len(subtasks), "agents": [a for a, _ in relevant_agents[:3]]}
        )
    ...
    async def _synthesize_results(
        self,
        root_task: AgentTask,
        subtask_outputs: List[AgentOutput]
    ) -> AgentOutput:
        """
        ...
        requires_approval, approval_reason = self.guardrail_engine.requires_human_approval(
            AgentOutput(
                task_id=root_task.task_id,
                agent_id=self.AGENT_ID,
