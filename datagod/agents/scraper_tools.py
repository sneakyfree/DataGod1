...
from datagod.agents.tool_registry import ToolPermission, tool_registry
...
class ScraperToolsAdapter:
    ...
    async def property_search(
        ...
        **kwargs,  # Accept additional params like 'query'
        ...
        try:
            ...
            return {"properties": [], "count": 0, "error": str(e), "source": "error"}
    ...
    async def entity_search(
        ...
        **kwargs,
        ...
        try:
            results = await self._run_scraper_search(
                search_type="entity",
                params={"name": name, "entity_type": entity_type, "state": state},
            )
    ...
    async def _run_scraper_search(
        self, search_type: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        result = await loop.run_in_executor(
            None, lambda: self._sync_search(search_type, params)
        )
    ...
    def _mock_property_search(
        self, address, parcel_id, county, state
    ) -> Dict[str, Any]:
    ...
    def _mock_court_search(
        self, party_name, case_type, county, state
    ) -> Dict[str, Any]:
