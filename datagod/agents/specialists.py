...
from .schemas import AgentTask, AgentOutput, EvidenceRef
...
class PropertyResearchAgent(BaseSpecialistAgent):
    ...
    RECORD_TYPES = [
        "deed", "mortgage", "assignment", "release", 
        "lien", "lis_pendens", "judgment", "ucc"
    ]
    ...
    async def process(self, task: AgentTask) -> AgentOutput:
        ...
        try:
            ...
            if search_result.get('success'):
                ...
                warnings.append(f"Property search: {search_result.get('error', 'Unknown error')}")
            ...
            self.log_action(
                task.task_id, "process_completed",
    ...
    def _extract_property_info(self, query: str, context: Dict[str, Any]) -> Dict[str, Any]:
...
class EntityResolutionAgent(BaseSpecialistAgent):
    ...
    async def process(self, task: AgentTask) -> AgentOutput:
        ...
        try:
            ...
            search_result = await self.execute_tool(
                "entity_search",
                {"name": entity_info['name'], "entity_type": entity_info.get('type', 'any')}
            )
            ...
            if search_result.get('success'):
                ...
                match_confidence = search_result.get('data', {}).get('match_confidence', 0.5)
                confidence = min(confidence + match_confidence * 0.4, 0.95)
            else:
                warnings.append(f"Entity search: {search_result.get('error', 'Unknown error')}")
            ...
            self.log_action(
                task.task_id, "process_completed",
                outputs={"entities_found": len(results.get('entity_search', {}).get('entities', [])), "confidence": confidence}
            )
    ...
    def _extract_entity_info(self, query: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'name' not in info:
            ...
            stop_words = {'find', 'search', 'look', 'for', 'the', 'a', 'an', 'who', 'is', 'are', 'was', 'owner', 'of'}
    ...
    def _classify_entity_type(self, name: str, search_results: Dict[str, Any]) -> str:
        ...
        company_indicators = ['llc', 'inc', 'corp', 'ltd', 'company', 'co.', 'trust', 'bank', 'mortgage']
    ...
    def _find_related_entities(self, search_results: Dict[str, Any]) -> List[Dict[str, Any]]:
...
class LienPriorityAgent(BaseSpecialistAgent):
    ...
    async def process(self, task: AgentTask) -> AgentOutput:
        ...
        try:
            ...
            lien_result = await self.execute_tool(
                "lien_search",
                {"property_id": property_id, "lien_types": list(self.PRIORITY_ORDER.keys())}
            )
            ...
            if lien_result.get('success'):
                ...
                evidence_refs.append(self.create_evidence_ref(
                    source="lien_search",
                    snippet=f"Found {len(liens)} liens"
                ))
                confidence = 0.6
            else:
                warnings.append(f"Lien search: {lien_result.get('error', 'Unknown error')}")
                liens = []
            ...
            results['clearance_requirements'] = self._generate_clearance_requirements(priority_stack)
            ...
            self.log_action(
                task.task_id, "process_completed",
                outputs={"liens_count": len(liens), "total_amount": results['totals'].get('total', 0)}
            )
    ...
    def _calculate_priority_stack(self, liens: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _calculate_totals(self, priority_stack: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            'total': total,
            'count': len(priority_stack),
            'by_type': by_type
        }
    
    def _generate_clearance_requirements(self, priority_stack: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
class RiskAssessmentAgent(BaseSpecialistAgent):
    ...
    async def process(self, task: AgentTask) -> AgentOutput:
        ...
        try:
            ...
            property_result = await self.execute_tool(
                "property_search",
                {"query": task.query, "property_id": property_id}
            )
            ...
            property_data = property_result.get('data', {}) if property_result.get('success') else {}
            lien_data = lien_result.get('data', {}) if lien_result.get('success') else {}
            ...
            distress_signals = self._identify_distress_signals(property_data, lien_data, red_flags)
            results['distress_signals'] = distress_signals
            
            # Step 5: Generate recommendations
            results['recommendations'] = self._generate_recommendations(red_flags, risk_score)
            ...
            self.log_action(
                task.task_id, "process_completed",
                outputs={"risk_score": risk_score['overall'], "red_flags_count": len(red_flags)}
            )
    ...
    def _detect_red_flags(self, property_data: Dict, lien_data: Dict) -> List[Dict[str, Any]]:
    ...
    def _identify_distress_signals(self, property_data: Dict, lien_data: Dict, red_flags: List) -> List[Dict]:
    ...
    def _generate_recommendations(self, red_flags: List, risk_score: Dict) -> List[Dict]:
...
class ComplianceCheckAgent(BaseSpecialistAgent):
    ...
    async def process(self, task: AgentTask) -> AgentOutput:
        ...
        try:
            ...
            self.log_action(
                task.task_id, "process_completed",
                outputs={"compliance_score": compliance_score, "checks_run": len(compliance_results)}
            )
