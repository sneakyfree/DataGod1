...
class APIIntegrationMetrics:
    ...
    def record_request(
        self, success: bool, response_time: float, error_type: str = None
    ):
...
class BaseAPIIntegration(ABC):
    ...
    def __init__(self, jurisdiction_id: int, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized {self.__class__.__name__} for jurisdiction {jurisdiction_id}"
        )
    ...
    def _check_rate_limit(self) -> bool:
        ...
        recent_requests = [
            ts for ts in self.request_timestamps if ts > now - timedelta(minutes=1)
        ]
    ...
    def start_run(self):
        ...
        self.current_run_id = self.scraper_logger.log_run_start(
            scraper_name=self.__class__.__name__, jurisdiction_id=self.jurisdiction_id
        )
    ...
    def make_request(self, method: str, endpoint: str, **kwargs) -> requests.Response:
        ...
        try:
            ...
            headers.update(
                self._get_auth_headers(method, endpoint, kwargs.get("data", {}))
            )
    ...
    def _get_auth_headers(
        self, method: str, endpoint: str, data: Any
    ) -> Dict[str, str]:
        ...
        metrics.update(
            {
                ...
                "config": {
                    ...
                    if not k.endswith("_secret") and not k.endswith("_key")
                },
            }
        )
        return metrics
