...
class NewYorkCountyAPI(BaseAPIIntegration):
    ...
    COUNTY_APIS = {
        'new-york': {  # Manhattan
            ...
            'features': ['property_search', 'deed_records', 'mortgage_records', 'ucc_filings', 'liens']
        },
        'kings': {  # Brooklyn
            'base_url': 'https://a836-acris.nyc.gov/api',
            'clerk_url': 'https://iapps.courts.state.ny.us/api',
            'features': ['property_search', 'deed_records', 'mortgage_records', 'liens']
        },
        'queens': {
            'base_url': 'https://a836-acris.nyc.gov/api',
    ...
    def __init__(self, jurisdiction_id: int, config: Dict[str, Any]):
        ...
        if self.county_config:
            ...
            self.base_url = f"https://www.{self.county_name.lower()}countyny.gov/api/records"
            self.clerk_url = ''
...
class NYCACRISApi(NewYorkCountyAPI):
    ...
    def search_by_borough_block_lot(self, borough: int, block: int, lot: int) -> List[Dict[str, Any]]:
