...
class ScraperOrchestrator:
    ...
    def __init__(self, max_workers: int = 5, persist_path: str = None, db_manager=None):
    ...
    def start(self, blocking: bool = False):
        ...
        if blocking:
            ...
            self._worker_thread = threading.Thread(
                target=self._run_workers, daemon=True
            )
    ...
    def _execute_task(self, task: ScrapingTask, worker_id: str) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"Worker {worker_id} executing task {task.task_id} for {task.jurisdiction_name}"
            )
            ...
            if hasattr(scraper, "end_run"):
                scraper.end_run(
                    status="success", items_scraped=len(records) if records else 0
                )
            ...
            logger.info(
                f"Task {task.task_id} completed with {len(records) if records else 0} records"
            )
            ...
            if task.retry_count < task.max_retries:
                ...
                self.task_queue.put(task)
                logger.info(
                    f"Task {task.task_id} queued for retry ({task.retry_count}/{task.max_retries})"
                )
            else:
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "workers": {
                ...
                "active": sum(
                    1 for w in self._workers.values() if w.current_task is not None
                ),
            },
        }
    ...
    def get_worker_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            worker_id: {
                ...
                "avg_time_per_task": stats.total_time_seconds
                / max(1, stats.tasks_completed),
            }
            for worker_id, stats in self._workers.items()
        }
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["start_time"]:
            ...
            ) / max(1, metrics["duration_seconds"] / 60)
            metrics["records_per_minute"] = metrics["total_records"] / max(
                1, metrics["duration_seconds"] / 60
            )
    ...
    def update_coverage(
        ...
        try:
            ...
            logger.info(
                f"Updated coverage for {fips_code}/{data_category}: {status.value}"
            )
    ...
    def get_coverage_gaps(
        self, state: str = None, data_category: str = None, min_population: int = 0
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            return self.db_manager.get_coverage_gaps(
                state=state, data_category=data_category, min_population=min_population
            )
        except Exception as e:
    ...
    def record_coverage_gap(
        ...
        try:
            ...
            with self.db_manager.get_session() as session:
                ...
                jurisdiction = (
                    session.query(Jurisdiction).filter_by(fips_code=fips_code).first()
                )
    ...
    def get_gaps_by_reason(
        self, reason: CoverageGapReason = None, state: str = None
    ) -> List[Dict[str, Any]]:
        """
            ...
            with self.db_manager.get_session() as session:
                ...
                for j in query.all():
                    ...
                    for category, cat_data in coverage.items():
                        if (
                            isinstance(cat_data, dict)
                            and cat_data.get("status") == "gap"
                        ):
                            ...
                            gaps.append(
                                {
                                    ...
                                    "alternative_source": cat_data.get(
                                        "alternative_source", ""
                                    ),
...
class ScraperScheduler:
    ...
    def add_schedule(
        ...
        logger.info(
            f"Added schedule {schedule_id} for {jurisdiction_name} (every {interval_hours}h)"
        )
            ...
            for schedule_id, schedule in self._schedules.items():
                if schedule.should_run():
                    try:
                        ...
                        logger.info(
                            f"Scheduled task {schedule_id} queued for {schedule.jurisdiction_name}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to queue scheduled task {schedule_id}: {e}"
                        )
...
class ScraperMonitor:
    ...
    def __init__(
        self, orchestrator: ScraperOrchestrator, scheduler: ScraperScheduler = None
    ):
        self.orchestrator = orchestrator
    ...
    def _calculate_summary(self) -> Dict[str, Any]:
        ...
        return {
            "total_tasks_processed": metrics.get("completed_tasks", 0)
            + metrics.get("failed_tasks", 0),
            "success_rate": (
                metrics.get("completed_tasks", 0)
                / max(
                    1,
                    metrics.get("completed_tasks", 0) + metrics.get("failed_tasks", 0),
                )
                * 100
            ),
            "total_records": metrics.get("total_records", 0),
            "uptime_seconds": metrics.get("duration_seconds", 0),
            "active_workers": metrics.get("queue_status", {})
            .get("workers", {})
            .get("active", 0),
        }
...
def get_orchestrator(
    max_workers: int = 5, persist_path: str = None, db_manager=None
) -> ScraperOrchestrator:
    """Factory function to create orchestrator"""
    return ScraperOrchestrator(
        max_workers=max_workers, persist_path=persist_path, db_manager=db_manager
    )
