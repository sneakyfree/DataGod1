"""
District of Columbia State API Integration
Auto-generated scraper for District of Columbia (DC) public records

Generated: 2025-12-31T17:41:38.058825
"""
...
COUNTY_APIS = {
    'district_of_columbia': {
        ...
        'base_url': 'https://otr.cfo.dc.gov/api',
        'property_endpoint': '/property/search',
        'deed_endpoint': '/recorder/deed/search',
        'lien_endpoint': '/recorder/lien/search',
        'requires_auth': True,
        'rate_limit': 60,
    },
}
...
class DistrictofColumbiaAPI(APIKeyAuthentication, BaseAPIIntegration):
    """
    API integration for District of Columbia (DC) public records.
    ...
    def __init__(self, jurisdiction_id: int, config: Dict[str, Any] = None):
        """
        Initialize the District of Columbia API integration.
    ...
    def authenticate(self) -> bool:
        """
        Authenticate with the District of Columbia API.
    ...
    def map_api_data_to_standard_format(self, api_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Map District of Columbia-specific API data to standard DataGod format.
    ...
    def search_business(self, query: Dict[str, Any], county: str = None) -> List[Dict[str, Any]]:
        """Search business records."""
        endpoint = "/dcra/business/search"
        if county:
            county_config = self.COUNTY_APIS.get(county.lower().replace(' ', '_'))
            if county_config:
                endpoint = county_config.get('business_endpoint', endpoint)
        ...
        for record in data.get('results', []):
            mapped = self.map_api_data_to_standard_format(record)
            mapped['record_type'] = 'business'
            results.append(mapped)

        return results


    def search_license(self, query: Dict[str, Any], county: str = None) -> List[Dict[str, Any]]:
        """Search license records."""
        endpoint = "/dcra/license/search"
        if county:
            county_config = self.COUNTY_APIS.get(county.lower().replace(' ', '_'))
            if county_config:
                endpoint = county_config.get('license_endpoint', endpoint)
...
def get_dc_api(jurisdiction_id: int, config: Dict[str, Any] = None) -> DistrictofColumbiaAPI:
    """
    Get a configured District of Columbia API instance.
    ...
    Returns:
        Configured DistrictofColumbiaAPI instance
    """
    return DistrictofColumbiaAPI(jurisdiction_id, config)
