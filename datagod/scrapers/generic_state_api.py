...
class GenericStateAPI(BaseAPIIntegration):
    ...
    def __init__(self, jurisdiction_id: int, config: Dict[str, Any]):
        ...
        logger.info(f"Initialized GenericStateAPI for {self.state_name} ({self.state_code})")
    ...
    def _search_endpoint(self, record_type: str, query: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            records = data.get('results', data.get(f'{record_type}s', data.get('data', [])))
    ...
    def _build_search_params(self, record_type: str, query: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in query.items():
            if key not in ['name', 'address', 'parcel_id', 'date_from', 'date_to',
                          'document_number', 'record_type'] and value:
                params[key] = value
...
def get_state_api(state_code: str, jurisdiction_id: int = 1,
                 config: Dict[str, Any] = None) -> GenericStateAPI:
    """
