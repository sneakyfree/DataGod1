...
class URLPatternDetector:
    ...
    def _extract_placeholders(
        self, url: str, county_name: str, state_code: str
    ) -> Tuple[str, List[str]]:
        """
        ...
        for variation in county_variations:
            if variation and variation in pattern.lower():
                pattern = re.sub(
                    re.escape(variation), "{county}", pattern, flags=re.IGNORECASE
                )
        ...
        if state_lower in pattern.lower():
            pattern = re.sub(
                rf"\b{state_lower}\b", "{state}", pattern, flags=re.IGNORECASE
            )
    ...
    def analyze_all_configs(self) -> Dict[str, List[URLPattern]]:
        ...
        for category, patterns in all_patterns.items():
            ...
            for pattern in patterns:
                pattern.confidence = (
                    pattern.match_count / total_matches if total_matches > 0 else 0
                )
    ...
    def generate_urls(
        self, category: str, county_name: str, state_code: str, limit: int = 3
    ) -> List[str]:
        """
...
def main():
    ...
    parser.add_argument(
        "--generate", type=str, help="Generate URLs for county (format: COUNTY,STATE)"
    )
        ...
        if args.generate:
            ...
            if len(parts) == 2:
                ...
                for category in ["property", "court", "business", "recorder"]:
                    urls = detector.generate_urls(
                        category, county.strip(), state.strip()
                    )
                    print(f"\n{category}: {urls}")
