...
class LosAngelesCountySheriff(SheriffInmateBase):
    ...
    def _parse_search_result(self, item: Dict[str, Any]) -> Optional[InmateRecord]:
        ...
        for charge_data in item.get("charges", []):
            ...
            charge = InmateCharge(
                ...
                charge_type=self._parse_charge_type(
                    charge_data.get("type", charge_data.get("description", ""))
                ),
                is_felony="FELONY"
                in str(
                    charge_data.get("type", charge_data.get("description", ""))
        ...
        # Parse bail
        bail_amount = self._parse_decimal(
            str(item.get("bail", item.get("bondAmount", "")))
        )
        bond_info = (
            BondInformation(
    ...
    def _parse_inmate_detail(self, data: Dict[str, Any]) -> InmateRecord:
        ...
        for charge_data in data.get("charges", data.get("chargeList", [])):
            ...
            charge = InmateCharge(
                ...
                severity=self._parse_charge_severity(
                    charge_data.get("class", charge_data.get("level", ""))
                ),
                ...
                is_felony="FELONY"
                in str(charge_data.get("type", charge_data.get("level", ""))).upper()
                or charge_data.get("felony", False),
                is_violent=self._is_violent_charge(charge_data.get("description", "")),
        ...
        bail_amount = self._parse_decimal(
            str(bail_data.get("amount", bail_data.get("total", "")))
        )
        bond_info = (
            BondInformation(
                bond_amount=bail_amount,
            ...
            if bail_amount or bail_data.get("status")
            else None
        )
        ...
        visitation = (
            VisitationInfo(
                ...
                video_visitation=visit_data.get(
                    "videoAvailable", True
                ),  # LA County has video visits
        ...
        return InmateRecord(
            ...
            scheduled_release=self._parse_date(
                data.get("scheduledRelease", data.get("projectedRelease", ""))
            ),
