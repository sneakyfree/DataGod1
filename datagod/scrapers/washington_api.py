...
class WashingtonCountyAPI(BaseAPIIntegration):
    ...
    def __init__(self, jurisdiction_id: int, config: Dict[str, Any]):
        ...
        if self.county_config:
            ...
            logger.warning(
                f"No specific API config for Washington county: {self.county_name}"
            )
            self.base_url = (
                f"https://www.{self.county_name.lower()}countywa.gov/assessor/api"
            )
            self.recorder_url = ""
    ...
    def _map_property_to_standard(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "amount": float(
                data.get("assessed_value", 0) or data.get("market_value", 0)
            ),
    ...
    def _map_deed_to_standard(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "amount": float(
                data.get("excise_tax_paid", 0) * 100 / 1.28
                if data.get("excise_tax_paid")
                else 0
            ),
