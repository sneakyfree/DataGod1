...
class BuildingPermitsAPI:
    ...
    async def search_by_address(
        self, state: str, city: str, address: str, limit: int = 25
    ) -> List[BuildingPermit]:
        """
        ...
        if jurisdiction_key in self.jurisdiction_sources:
            return await self._search_jurisdiction(
                session, jurisdiction_key, address=address, limit=limit
            )

        # Try county-level if city not found
        county_key = (
            f"{state}_{city.upper().replace(' ', '_').replace('CITY_OF_', '')}_COUNTY"
        )
    ...
    async def search_by_permit_number(
        self, state: str, city: str, permit_number: str
        ...
        permits = await self._search_jurisdiction(
            session, jurisdiction_key, permit_number=permit_number, limit=1
        )
        return permits[0] if permits else None

    async def search_by_owner(
        self, state: str, city: str, owner_name: str, limit: int = 50
    ) -> List[BuildingPermit]:
        """
        ...
        return await self._search_jurisdiction(
            session, jurisdiction_key, owner_name=owner_name, limit=limit
        )
    ...
    async def search_by_contractor(
        ...
        state = state.upper()
        logger.info(
            f"Searching permits in {city}, {state} for contractor: {contractor_name}"
        )
        ...
        return await self._search_jurisdiction(
            session, jurisdiction_key, contractor_name=contractor_name, limit=limit
        )
    ...
    async def search_high_value_permits(
        ...
        permits = await self._search_jurisdiction(
            session, jurisdiction_key, permit_type=permit_type, limit=limit * 2
        )
        ...
        try:
            async with session.get(api_url, params=params) as response:
                if response.status == 200:
                    ...
                    permits = self._parse_open_data_results(
                        data, state, jurisdiction_name, limit
                    )
    ...
    def _parse_open_data_results(
        self, data: List[Dict[str, Any]], state: str, jurisdiction: str, limit: int
    ) -> List[BuildingPermit]:
        ...
        for item in data[:limit]:
            try:
                ...
                app_date = self._parse_date(
                    ...
                    or item.get("permit_creation_date")
                )
                ...
                permit = BuildingPermit(
                    ...
                    or item.get("permit_no")
                    or item.get("job_number")
                    ...
                    valuation=self._parse_float(
                        ...
                        or item.get("job_value")
                        or item.get("total_cost")
                    ),
                    permit_fee=self._parse_float(
                        item.get("permit_fee") or item.get("fee")
                    ),
                    total_fees=self._parse_float(
                        item.get("total_fee") or item.get("total_fees")
                    ),
                    issue_date=self._parse_date(
                        item.get("issue_date") or item.get("issued_date")
                    ),
                    expiration_date=self._parse_date(
                        item.get("expiration_date") or item.get("expire_date")
                    ),
                    final_date=self._parse_date(
                        item.get("final_date") or item.get("finaled_date")
                    ),
                ...
                if contractor_name:
                    permit.contractor = Contractor(
                        ...
                        license_number=item.get("contractor_license")
            ...
            try:
                ...
                async with session.post(search_url, data=data) as response:
                    if response.status == 200:
                        ...
                        permits = self._parse_web_results(
                            html, state, jurisdiction_name, limit
                        )
                        if permits:
                            break
            except Exception as e:
    ...
    def _parse_web_results(
        self, html: str, state: str, jurisdiction: str, limit: int
    ) -> List[BuildingPermit]:
            ...
            if len(cells) >= 3:
                try:
                    ...
                    for text in text_content:
                        ...
                        if re.match(r"^[A-Z0-9]{4,}-", text) or re.match(
                            r"^\d{4,}[A-Z]", text
                        ):
                            permit_number = text
    ...
    def _classify_permit_type(self, type_text: str) -> PermitType:
        ...
        if "new construct" in type_lower or "new build" in type_lower:
            ...
            "hvac" in type_lower
            or "air condition" in type_lower
            or "heating" in type_lower
        ):
            return PermitType.HVAC
        elif "roof" in type_lower:
            return PermitType.ROOFING
        elif (
            "solar" in type_lower or "pv" in type_lower or "photovoltaic" in type_lower
        ):
            ...
            "certificate of occupancy" in type_lower
            or "c of o" in type_lower
            or "c/o" in type_lower
        ):
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        jurisdictions_with_api = [
            j for j, c in self.jurisdiction_sources.items() if c.get("api_available")
        ]
...
def search_permits_by_address(
    address: str, state: str, city: str, limit: int = 25
) -> List[Dict[str, Any]]:
    """
...
def search_permits_by_owner(
    owner_name: str, state: str, city: str, limit: int = 50
) -> List[Dict[str, Any]]:
    """
...
def search_permits_by_contractor(
    ...
    async def _search():
        async with BuildingPermitsAPI() as api:
            permits = await api.search_by_contractor(
                state, city, contractor_name, license_number, limit
            )
            return [p.to_dict() for p in permits]
...
def get_permit(permit_number: str, state: str, city: str) -> Optional[Dict[str, Any]]:
    """
        ...
        async with BuildingPermitsAPI() as api:
            ...
            if permit_type:
                try:
                    ...
                    pass
            permits = await api.search_recent_permits(
                state, city, type_enum, days_back, limit
            )
            return [p.to_dict() for p in permits]
...
def get_high_value_permits(
    state: str, city: str, min_value: float = 1000000, limit: int = 50
) -> List[Dict[str, Any]]:
    """
    ...
    async def _search():
        async with BuildingPermitsAPI() as api:
            permits = await api.search_high_value_permits(
                state, city, min_value, None, limit
            )
            return [p.to_dict() for p in permits]
