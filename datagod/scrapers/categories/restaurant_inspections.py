...
class RestaurantInspectionsAPI:
    ...
    def _parse_result(
        self, result_str: str, score: Optional[int] = None
    ) -> InspectionResult:
    ...
    async def search_by_restaurant_name(
        self, name: str, jurisdiction: str, limit: int = 50
    ...
    async def search_by_address(
        self, address: str, city: str, state: str, limit: int = 50
    ) -> List[InspectionRecord]:
    ...
    async def get_recent_inspections(
        self, jurisdiction: str, days: int = 30, limit: int = 100
    ) -> List[InspectionRecord]:
    ...
    async def get_failed_inspections(
        self, jurisdiction: str, days: int = 90, limit: int = 100
    ) -> List[InspectionRecord]:
    ...
    async def get_inspection_by_id(
        self, inspection_id: str, jurisdiction: str
    ...
    async def get_establishment_history(
        self, permit_number: str, jurisdiction: str, limit: int = 20
    ) -> List[InspectionRecord]:
    ...
    def _parse_open_data_record(
        self, record: Dict, jurisdiction: str
    ) -> Optional[InspectionRecord]:
        ...
        try:
            ...
            if record.get("violation_description"):
                violation = Violation(
                    ...
                    severity=self._parse_violation_severity(
            ...
            return InspectionRecord(
                ...
                inspection_type=self._parse_inspection_type(
                    record.get("inspection_type", "")
                ),
                result=self._parse_result(
                    record.get("results") or record.get("grade_description", ""), score
                ),
                score=score,
                grade=grade,
                ...
                in (record.get("results", "") or "").lower(),
                source=JURISDICTION_INSPECTION_SOURCES.get(jurisdiction, {}).get(
                    "name", jurisdiction
                ),
                source_url=JURISDICTION_INSPECTION_SOURCES.get(jurisdiction, {}).get(
                    "url"
                ),
            )
        except Exception as e:
    ...
    async def search_la_county_by_zip(
        self, zip_code: str, limit: int = 100
    ) -> List[InspectionRecord]:
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        api_available = sum(
            1
            for j in JURISDICTION_INSPECTION_SOURCES.values()
            if j.get("api_available")
        )
        ...
        return {
            ...
            "scoring_systems": {
                "numeric": sum(
                    1
                    for j in JURISDICTION_INSPECTION_SOURCES.values()
                    if j.get("scoring_system") == "numeric"
                ),
                "pass_fail": sum(
                    1
                    for j in JURISDICTION_INSPECTION_SOURCES.values()
                    if j.get("scoring_system") == "pass_fail"
                ),
                "violation_points": sum(
                    1
                    for j in JURISDICTION_INSPECTION_SOURCES.values()
                    if j.get("scoring_system") == "violation_points"
                ),
                "demerits": sum(
                    1
                    for j in JURISDICTION_INSPECTION_SOURCES.values()
                    if j.get("scoring_system") == "demerits"
                ),
            },
            "jurisdictions_with_grades": sum(
                1
                for j in JURISDICTION_INSPECTION_SOURCES.values()
                if j.get("grade_system")
            ),
        }
...
def search_inspections_by_restaurant(
    name: str, jurisdiction: str, limit: int = 50
) -> List[Dict[str, Any]]:
...
def search_inspections_by_address(
    address: str, city: str, state: str, limit: int = 50
) -> List[Dict[str, Any]]:
...
def get_recent_inspections(
    jurisdiction: str, days: int = 30, limit: int = 100
) -> List[Dict[str, Any]]:
...
def get_failed_inspections(
    jurisdiction: str, days: int = 90, limit: int = 100
) -> List[Dict[str, Any]]:
...
def get_establishment_history(
    permit_number: str, jurisdiction: str, limit: int = 20
) -> List[Dict[str, Any]]:
    """Synchronous wrapper for getting establishment inspection history"""

    async def _get():
        async with RestaurantInspectionsAPI() as api:
            results = await api.get_establishment_history(
                permit_number, jurisdiction, limit
            )
            return [r.to_dict() for r in results]
...
if __name__ == "__main__":
    ...
    print(
        f"  With API Access: {stats['jurisdictions_with_api']} ({stats['api_coverage_percent']}%)"
    )
