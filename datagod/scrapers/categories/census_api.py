...
class CensusApiScraper:
    ...
    def __init__(self, api_key: str = None):
        ...
        logger.info(
            f"CensusApiScraper initialized (API key: {'present' if self.api_key else 'not set'})"
        )
            ...
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    return await self._make_request(
                        url, session if not close_session else None
                    )
                else:
                    logger.error(
                        f"Census API error {response.status}: {await response.text()}"
                    )
                    return []
    ...
    def _parse_response(
        self, data: List[List[Any]], variable_names: Dict[str, str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_demographics(
        ...
        for row in parsed:
            ...
            record = CensusRecord(
                ...
                variables={
                    k: v for k, v in row.items() if k not in ["NAME", "state", "county"]
                },
                source_url=url,
                raw_data=row,
            )
    ...
    async def get_income_data(
        self, state_fips: str, county_fips: str = "*", year: int = 2022
    ) -> List[CensusRecord]:
        """
        ...
        for row in parsed:
            ...
            record = CensusRecord(
                ...
                variables={
                    ...
                    if k not in ["NAME", "state", "county", "tract"]
                },
                source_url=url,
                raw_data=row,
            )
    ...
    async def get_population_estimates(
        self, state_fips: str, county_fips: str = "*", vintage: int = 2023
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    ...
                    if response.status != 200:
                        logger.warning(
                            f"Could not fetch variables list: {response.status}"
                        )
                        return []
                    ...
                    for var_code, var_info in variables_dict.items():
                        ...
                        if (
                            keyword_lower in label.lower()
                            or keyword_lower in concept.lower()
                        ):
                            results.append(
                                {
                                    "code": var_code,
    ...
    async def get_geography_codes(
        self, geography_level: GeographyLevel, state_fips: str = "*"
    ) -> List[Dict[str, str]]:
        """
        ...
        if geography_level == GeographyLevel.STATE:
            ...
            logger.warning(
                f"Geography level {geography_level} not supported for code lookup"
            )
            return []
    ...
    async def get_decennial_population(
        self, state_fips: str, county_fips: str = "*", census_year: int = 2020
    ) -> List[CensusRecord]:
        """
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rate_limit": (
                self.config["rate_limit_no_key"] if not self.api_key else "unlimited"
            ),
...
def get_county_demographics(
    ...
    records = loop.run_until_complete(
        scraper.get_demographics(state_fips, county_fips, year)
    )
...
def get_county_population(
    ...
    loop = asyncio.get_event_loop()
    records = loop.run_until_complete(
        scraper.get_population(state_fips, county_fips, year)
    )
    return [r.to_dict() for r in records]
