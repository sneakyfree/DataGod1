...
class MechanicLiensAPI:
    ...
    async def search_by_property_address(
        self, address: str, city: str, state: str, county: Optional[str] = None
    ...
    async def search_by_property_owner(
        self, owner_name: str, state: str, county: Optional[str] = None
    ) -> List[MechanicLienRecord]:
    ...
    async def search_by_claimant(
        self, claimant_name: str, state: str, county: Optional[str] = None
    ) -> List[MechanicLienRecord]:
    ...
    async def search_by_apn(
        self, apn: str, state: str, county: str
    ) -> List[MechanicLienRecord]:
    ...
    async def get_lien_by_document_number(
        self, document_number: str, state: str, county: str
    ...
    async def get_expiring_liens(
        ...
        logger.info(
            f"State foreclosure deadline: {foreclosure_deadline} days from filing"
        )
    ...
    def get_filing_deadline(
        self, state: str, claimant_type: str = "contractor"
        ...
        return {
            ...
            "filing_deadline_sub_days": info.get(
                "filing_deadline_sub_days", info.get("filing_deadline_days")
            ),
            "foreclosure_deadline_days": info.get("foreclosure_deadline_days"),
            "preliminary_notice_required": info.get(
                "preliminary_notice_required", False
            ),
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        with_preliminary_notice = sum(
            1 for s in STATE_LIEN_LAWS.values() if s.get("preliminary_notice_required")
        )
        with_stop_notice = sum(
            1 for s in STATE_LIEN_LAWS.values() if s.get("stop_notice_available")
        )

        # Filing deadline ranges
        deadlines = [s["filing_deadline_days"] for s in STATE_LIEN_LAWS.values()]
        foreclosure_deadlines = [
            s["foreclosure_deadline_days"] for s in STATE_LIEN_LAWS.values()
        ]
...
def search_liens_by_property(
    address: str, city: str, state: str, county: Optional[str] = None
) -> List[Dict[str, Any]]:
...
def search_liens_by_owner(
    owner_name: str, state: str, county: Optional[str] = None
) -> List[Dict[str, Any]]:
...
def search_liens_by_claimant(
    claimant_name: str, state: str, county: Optional[str] = None
) -> List[Dict[str, Any]]:
...
def get_lien_by_document(
    document_number: str, state: str, county: str
) -> Optional[Dict[str, Any]]:
    """Synchronous wrapper for getting lien by document number"""

    async def _get():
        async with MechanicLiensAPI() as api:
            result = await api.get_lien_by_document_number(
                document_number, state, county
            )
...
def get_filing_deadline(
    state: str, claimant_type: str = "contractor"
) -> Optional[Dict[str, Any]]:
    """Get filing deadline for a state"""
    api = MechanicLiensAPI()
...
if __name__ == "__main__":
    ...
    print(
        f"  States Requiring Preliminary Notice: {stats['states_requiring_preliminary_notice']}"
    )
