...
class DBAFilingsAPI:
    ...
    def _parse_status(self, status_str: str, expiration_date: Optional[date] = None) -> DBAStatus:
    ...
    def _parse_business_structure(self, structure_str: str) -> BusinessStructure:
        ...
        elif 'married' in structure_str or 'husband' in structure_str or 'wife' in structure_str:
    ...
    async def get_expiring_filings(
        ...
        logger.info(f"Searching for DBAs expiring within {days_until_expiration} days in {state}")
    ...
    def get_filing_requirements(self, state: str) -> Dict[str, Any]:
        ...
        if info.get('publication_required'):
            requirements['publication_info'] = {
                ...
                'notes': info.get('notes', 'Check with filing office for specific requirements'),
            }
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        county_level = sum(1 for s in STATE_DBA_INFO.values() if s['filing_level'] == 'county')
        state_level = sum(1 for s in STATE_DBA_INFO.values() if s['filing_level'] == 'state')
        town_level = sum(1 for s in STATE_DBA_INFO.values() if s['filing_level'] in ['town', 'city_town'])
        ...
        with_expiration = sum(1 for s in STATE_DBA_INFO.values() if s.get('duration_years'))
        with_publication = sum(1 for s in STATE_DBA_INFO.values() if s.get('publication_required'))
