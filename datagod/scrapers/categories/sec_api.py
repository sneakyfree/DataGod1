    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "filing_type": (
                ...
                if isinstance(self.filing_type, FilingType)
                else self.filing_type
            ),
            "filing_date": self.filing_date.isoformat() if self.filing_date else None,
            "accepted_date": (
                self.accepted_date.isoformat() if self.accepted_date else None
            ),
...
class SECApiScraper:
    ...
    async def search_company(
        self, company_name: str = "", cik: str = "", ticker: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        if cik:
            ...
            if data:
                return {
                    ...
                    "ticker": (
                        data.get("tickers", [""])[0] if data.get("tickers") else ""
                    ),
                    "exchange": (
        ...
        for i, acc_num in enumerate(accession_numbers):
            ...
            filing = SECFiling(
                ...
                primary_doc_description=(
                    primary_descriptions[i] if i < len(primary_descriptions) else ""
                ),
                ticker=ticker,
                exchange=exchange,
                sic_code=sic_code,
    ...
    async def get_institutional_holdings(
        self, cik: str = "", ticker: str = "", holder_cik: str = "", quarter: str = ""
    ) -> List[Dict[str, Any]]:
        """
        ...
        if holder_cik:
            ...
            filings = await self.get_company_filings(
                cik=holder_cik, filing_type=FilingType.FORM_13F, limit=10
            )
    ...
    async def get_company_facts(self, cik: str, concept: str = "") -> Dict[str, Any]:
        """
        ...
        if concept:
            ...
            url = (
                f"{self.endpoints['company_facts']['base_url']}CIK{formatted_cik}.json"
            )
    ...
    async def get_recent_filings(
        self, filing_type: FilingType = None, limit: int = 100
    ) -> List[SECFiling]:
        """
        ...
        if filing_type:
            ...
            return await self.search_filings_fulltext(
                query="*", start_date=thirty_days_ago, end_date=today, limit=limit
            )
    ...
    async def get_beneficial_ownership(
        ...
        for filing_type in [FilingType.FORM_13D, FilingType.FORM_13G]:
            filings = await self.get_company_filings(
                cik=cik, filing_type=filing_type, limit=50
            )
...
async def search_sec_filings_async(
    cik: str = "", ticker: str = "", filing_type: str = "", **kwargs
) -> List[Dict[str, Any]]:
