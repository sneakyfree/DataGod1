...
class ForeclosureRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "original_loan_date": (
                self.original_loan_date.isoformat() if self.original_loan_date else None
            ),
            "mortgage_info": (
                self.mortgage_info.to_dict() if self.mortgage_info else None
            ),
...
class ForeclosuresAPI:
    ...
    async def search_by_address(
        ...
        # Search HUD homes
        hud_records = await self._search_hud_homes(
            session, state, city, zip_code, limit
        )
        records.extend(hud_records)

        # Search Fannie Mae HomePath
        homepath_records = await self._search_homepath(
            session, state, city, zip_code, limit
        )
        records.extend(homepath_records)

        # Search county-specific sources
        county_records = await self._search_county_foreclosures(
            session, state, address, city, limit
        )
    ...
    async def search_by_county(
        ...
        if county_key in self.county_sources:
            records = await self._search_specific_county(
                session, county_key, stage, date_from, date_to, limit
            )
        else:
            # Generic county search
            records = await self._search_generic_county(
                session, state, county, stage, date_from, date_to, limit
            )

        return records

    async def search_by_borrower(
        self, state: str, borrower_name: str, county: str = None, limit: int = 25
    ) -> List[ForeclosureRecord]:
        """
        ...
        if county:
            records = await self._search_county_by_name(
                session, state, county, borrower_name, limit
            )
        else:
            # Search multiple major counties in state
            major_counties = self._get_major_counties(state)
            for county_name in major_counties[:5]:
                county_records = await self._search_county_by_name(
                    session, state, county_name, borrower_name, limit // 5
                )
                records.extend(county_records)
    ...
    async def search_scheduled_auctions(
        ...
        auction_records = await self._search_auction_sites(
            session, state, county, date_from, date_to, limit
        )
        ...
        records = [
            r
            for r in records
            if r.foreclosure_stage == ForeclosureStage.AUCTION_SCHEDULED
        ]
    ...
    async def search_reo_properties(
        ...
        # Search HUD homes
        hud_records = await self._search_hud_homes(
            session, state, city, zip_code, limit // 3
        )
        records.extend(hud_records)
        ...
        homesteps_records = await self._search_homesteps(
            session, state, city, zip_code, limit // 3
        )
        ...
        if max_price:
            records = [
                r
                for r in records
                if r.estimated_value and r.estimated_value <= max_price
            ]
    ...
    def _parse_hud_results(
        self, html: str, state: str, limit: int
    ) -> List[ForeclosureRecord]:
        ...
        listings = soup.select(
            '.property-listing, .listing-item, div[class*="property"]'
        )
        ...
        for listing in listings[:limit]:
            try:
                ...
                price_elem = listing.select_one(
                    '.price, .list-price, span[class*="price"]'
                )
                ...
                # Extract case number
                case_elem = listing.select_one(
                    '.case-number, .hud-case, span[class*="case"]'
                )
                case_number = (
                    case_elem.get_text(strip=True)
                    if case_elem
                    else f"HUD-{datetime.now().strftime('%Y%m%d')}"
                )
    ...
    def _parse_homepath_json(
        self, data: Dict[str, Any], state: str, limit: int
    ) -> List[ForeclosureRecord]:
        ...
        for listing in listings[:limit]:
            try:
                record = ForeclosureRecord(
                    case_number=listing.get(
                        "listingId", f"FNMA-{datetime.now().strftime('%Y%m%d')}"
                    ),
                    state=state,
                    ...
                    property_type=self._classify_property_type(
    ...
    def _parse_homesteps_json(
        ...
        for listing in listings[:limit]:
            try:
                record = ForeclosureRecord(
                    case_number=listing.get(
                        "propertyId", f"FHLMC-{datetime.now().strftime('%Y%m%d')}"
                    ),
                    state=state,
                    county=listing.get("county", ""),
                    recording_date=date.today(),
    ...
    async def _search_specific_county(
        ...
        if auction_url:
            auction_records = await self._search_auction_url(
                session, auction_url, state, county_name, limit
            )
            records.extend(auction_records)
    ...
    async def _search_generic_county(
        ...
        aggregator_records = await self._search_auction_aggregators(
            session, state, county, limit
        )
    ...
    async def _search_auction_aggregators(
        ...
        # Bid4Assets
        bid4assets_records = await self._search_bid4assets(
            session, state, county, limit // 2
        )
        records.extend(bid4assets_records)

        # Auction.com
        auction_com_records = await self._search_auction_com(
            session, state, county, limit // 2
        )
    ...
    def _parse_bid4assets_results(
        self, html: str, state: str, county: str, limit: int
    ) -> List[ForeclosureRecord]:
        ...
        for listing in listings[:limit]:
            try:
                ...
                opening_bid = self._parse_price(
                    bid_elem.get_text(strip=True) if bid_elem else ""
                )
    ...
    async def _search_auction_com(
        ...
        try:
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    html = await response.text()
                    records = self._parse_auction_com_results(
                        html, state, county, limit
                    )
        except Exception as e:
    ...
    def _parse_auction_com_results(
        self, html: str, state: str, county: str, limit: int
    ) -> List[ForeclosureRecord]:
        ...
        try:
            async with session.get(auction_url) as response:
                if response.status == 200:
                    ...
                    listings = soup.select(
                        '.auction, .listing, .property, tr[class*="auction"]'
                    )
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        non_judicial_states = [
            s
            for s, c in self.state_configs.items()
            if c.get("process") == "non_judicial"
        ]
        judicial_states = [
            s for s, c in self.state_configs.items() if c.get("process") == "judicial"
        ]
...
def search_foreclosures_by_address(
    state: str, address: str, city: str = None, zip_code: str = None, limit: int = 25
) -> List[Dict[str, Any]]:
    """
...
def search_foreclosures_by_county(
    ...
    async def _search():
        async with ForeclosuresAPI() as api:
            ...
            records = await api.search_by_county(
                state, county, stage_enum, None, from_date, to_date, limit
            )
            return [r.to_dict() for r in records]
...
def search_foreclosure_auctions(
    state: str, county: str = None, limit: int = 50
) -> List[Dict[str, Any]]:
    """
    ...
    async def _search():
        async with ForeclosuresAPI() as api:
            records = await api.search_scheduled_auctions(
                state, county, None, None, limit
            )
            return [r.to_dict() for r in records]
...
def search_reo_properties(
    ...
    async def _search():
        async with ForeclosuresAPI() as api:
            records = await api.search_reo_properties(
                state, city, zip_code, max_price, limit
            )
            return [r.to_dict() for r in records]
