...
class HuntingFishingAPI:
    ...
    async def search_by_license_number(
        self, license_number: str, state: str
    ...
    async def get_harvest_data(
        self, state: str, species: str, year: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def get_license_requirements(
        self, state: str, license_type: str
    ) -> Dict[str, Any]:
    ...
    async def verify_license(
        self, license_number: str, state: str, date_of_birth: Optional[date] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        with_api = sum(
            1 for s in STATE_WILDLIFE_AGENCIES.values() if s.get("api_available")
        )
        with_electronic = sum(
            1 for s in STATE_WILDLIFE_AGENCIES.values() if s.get("electronic_license")
        )
        with_harvest_reporting = sum(
            1 for s in STATE_WILDLIFE_AGENCIES.values() if s.get("harvest_reporting")
        )
        with_draw = sum(
            1 for s in STATE_WILDLIFE_AGENCIES.values() if s.get("draw_system")
        )
        ...
        return {
            ...
            "electronic_license_percent": round(
                with_electronic / total_states * 100, 1
            ),
            "notes": "Most states do not provide public API access to individual license records",
        }

    async def search_violations(
        self, state: str, last_name: Optional[str] = None, year: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
...
def search_licenses_by_name(
    last_name: str, first_name: str, state: str, license_year: Optional[str] = None
) -> List[Dict[str, Any]]:
    """Synchronous wrapper for searching licenses by name"""

    async def _search():
        async with HuntingFishingAPI() as api:
            results = await api.search_by_name(
                last_name, first_name, state, license_year
            )
...
def get_license_by_number(license_number: str, state: str) -> Optional[Dict[str, Any]]:
...
def get_harvest_data(
    state: str, species: str, year: Optional[int] = None
) -> Dict[str, Any]:
...
def get_license_requirements(state: str, license_type: str) -> Dict[str, Any]:
...
def verify_license(
    license_number: str, state: str, date_of_birth: Optional[date] = None
) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    print(
        f"  With API Access: {stats['states_with_api']} ({stats['api_coverage_percent']}%)"
    )
    print(
        f"  Electronic Licensing: {stats['states_with_electronic_license']} ({stats['electronic_license_percent']}%)"
    )
