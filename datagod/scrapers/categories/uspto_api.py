...
class PatentRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "patent_type": (
                self.patent_type.value
                if isinstance(self.patent_type, PatentType)
                else self.patent_type
            ),
            "title": self.title,
            ...
            "status": (
                self.status.value
                if isinstance(self.status, PatentStatus)
                else self.status
            ),
            ...
            "priority_date": (
                self.priority_date.isoformat() if self.priority_date else None
            ),
...
class TrademarkRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "status": (
                self.status.value
                if isinstance(self.status, TrademarkStatus)
                else self.status
            ),
            "filing_date": self.filing_date.isoformat() if self.filing_date else None,
            "registration_date": (
            ...
            "abandonment_date": (
                self.abandonment_date.isoformat() if self.abandonment_date else None
            ),
            "cancellation_date": (
                self.cancellation_date.isoformat() if self.cancellation_date else None
            ),
            "renewal_date": (
...
class USPTOApiScraper:
    ...
    async def search_patents(
        ...
        """
        logger.info(
            f"Searching patents: {query or inventor or assignee or patent_number}"
        )
        patents = []
    ...
    async def get_patent_by_number(self, patent_number: str) -> Optional[PatentRecord]:
        """
    ...
    async def search_trademarks(
        ...
        if serial_number:
            ...
            logger.warning(
                "USPTO trademark full-text search requires TESS web interface"
            )

        return trademarks

    async def get_trademark_by_serial(
        self, serial_number: str, api_key: str = None
        ...
        tm_data = (
            data["trademarkBag"]
            .get("trademarkTransactionBag", {})
            .get("trademarkTransaction", [{}])[0]
        )
        if not tm_data:
            return None

        tm_bag = (
            tm_data.get("trademarkBag", {}).get("trademark", [{}])[0]
            if tm_data.get("trademarkBag")
            else {}
        )
        ...
        # Extract owner info
        applicant = (
            tm_bag.get("applicantBag", {}).get("applicant", [{}])[0]
            if tm_bag.get("applicantBag")
            else {}
        )
        owner_name = applicant.get("organizationName") or applicant.get(
            "personName", ""
        )
        ...
        int_classes = [
            int(gs.get("classNumber", 0)) for gs in gs_bag if gs.get("classNumber")
        ]
        ...
        record = TrademarkRecord(
            ...
            live_dead=(
                "LIVE"
                if tm_status not in [TrademarkStatus.DEAD, TrademarkStatus.ABANDONED]
                else "DEAD"
            ),
            source_url=f"https://tsdr.uspto.gov/#caseNumber={clean_serial}&caseSearchType=US_APPLICATION&caseType=DEFAULT",
            raw_data=data,
    ...
    async def get_patent_assignments(
        self, patent_number: str = "", assignee: str = "", assignor: str = ""
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        logger.info(
            f"Getting patent assignments for {patent_number or assignee or assignor}"
        )
        assignments = []
    ...
    async def get_inventor_patents(
        self, inventor_name: str, state: str = "", country: str = ""
    ) -> List[PatentRecord]:
        """
    ...
    async def get_assignee_patents(
        self, assignee_name: str, start_date: date = None, end_date: date = None
    ) -> List[PatentRecord]:
        """
        ...
        return await self.search_patents(
            assignee=assignee_name, start_date=start_date, end_date=end_date, limit=100
        )

    async def get_patent_citations(
        self, patent_number: str, direction: str = "both"
    ) -> Dict[str, List[str]]:
        """
    ...
    async def get_ptab_proceedings(
        self, patent_number: str = "", party_name: str = "", proceeding_type: str = ""
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_recent_patents(
        self, patent_type: PatentType = None, cpc_section: str = "", limit: int = 100
    ) -> List[PatentRecord]:
        """
...
async def search_trademarks_async(
    mark_text: str = "", **kwargs
) -> List[Dict[str, Any]]:
