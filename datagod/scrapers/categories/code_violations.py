...
class CodeViolationRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "complaint_date": (
                self.complaint_date.isoformat() if self.complaint_date else None
            ),
            "compliance_deadline": (
...
class CodeViolationsAPI:
    ...
    async def search_by_owner(
        self, owner_name: str, city: str, state: str
    ) -> List[CodeViolationRecord]:
    ...
    async def search_by_case_number(
        self, case_number: str, city: str, state: str
    ...
    async def get_open_violations(
        self, city: str, state: str, violation_type: Optional[ViolationType] = None
    ) -> List[CodeViolationRecord]:
    ...
    async def get_recent_violations(
        self, city: str, state: str, days: int = 30
    ) -> List[CodeViolationRecord]:
    ...
    def _parse_open_data_record(
        ...
        try:
            ...
            if date_str:
                if "T" in date_str:
                    complaint_date = datetime.fromisoformat(
                        date_str.replace("Z", "")
                    ).date()
                else:
            ...
            return CodeViolationRecord(
                ...
                violation_type=self._parse_violation_type(
                    record.get("violation_type", "")
                ),
                status=self._parse_status(record.get("status", "")),
                description=record.get("description")
                or record.get("violation_description", ""),
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        with_api = sum(
            1 for c in CITY_CODE_ENFORCEMENT.values() if c.get("api_available")
        )
...
def get_violation_by_case(
    case_number: str, city: str, state: str
...
def get_open_violations(
    city: str, state: str, violation_type: Optional[str] = None
) -> List[Dict[str, Any]]:
...
def get_recent_violations(
    city: str, state: str, days: int = 30
) -> List[Dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    print(
        f"  Cities with API: {stats['cities_with_api']} ({stats['api_coverage_percent']}%)"
    )
