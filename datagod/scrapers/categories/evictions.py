...
class EvictionJudgment:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'stay_expiration': self.stay_expiration.isoformat() if self.stay_expiration else None,
        }
...
class EvictionRecord:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'writ_issued_date': self.writ_issued_date.isoformat() if self.writ_issued_date else None,
            'writ_type': self.writ_type,
            'writ_executed_date': self.writ_executed_date.isoformat() if self.writ_executed_date else None,
            'lockout_date': self.lockout_date.isoformat() if self.lockout_date else None,
...
class EvictionsAPI:
    ...
    async def search_by_defendant(
        ...
        if county:
            ...
            if county_key in self.county_sources:
                return await self._search_county(session, county_key, defendant_name=defendant_name, date_from=date_from, date_to=date_to, limit=limit)
        ...
        for county_key in state_counties[:5]:
            county_records = await self._search_county(session, county_key, defendant_name=defendant_name, date_from=date_from, date_to=date_to, limit=limit // 5)
    ...
    async def search_by_plaintiff(
        ...
        if county:
            county_key = f"{state}_{county.upper().replace(' ', '_').replace('COUNTY', '').strip()}"
            if county_key in self.county_sources:
                return await self._search_county(session, county_key, plaintiff_name=plaintiff_name, date_from=date_from, date_to=date_to, limit=limit)
        ...
        for county_key in state_counties[:5]:
            county_records = await self._search_county(session, county_key, plaintiff_name=plaintiff_name, date_from=date_from, date_to=date_to, limit=limit // 5)
            records.extend(county_records)
    ...
    async def search_by_address(
        ...
        if county:
            county_key = f"{state}_{county.upper().replace(' ', '_').replace('COUNTY', '').strip()}"
            if county_key in self.county_sources:
                return await self._search_county(session, county_key, address=address, limit=limit)
        ...
        for county_key in state_counties[:3]:
            county_records = await self._search_county(session, county_key, address=address, limit=limit // 3)
            records.extend(county_records)
    ...
    async def search_by_case_number(
        ...
        records = await self._search_county(session, county_key, case_number=case_number, limit=1)
    ...
    async def search_recent_filings(
        ...
        return await self._search_county(session, county_key, date_from=date_from, limit=limit)
    ...
    async def _search_county(
        ...
        county_name = config.get('name', '').replace(' County', '').replace(' Superior Court', '').replace(' Justice Courts', '')

        # Route to specific implementations
        if county_key == 'TX_HARRIS':
            return await self._search_harris_county(session, defendant_name, plaintiff_name, case_number, date_from, date_to, limit)
        elif county_key == 'FL_MIAMI_DADE':
            return await self._search_miami_dade(session, defendant_name, plaintiff_name, case_number, date_from, date_to, limit)
        elif county_key == 'IL_COOK':
            return await self._search_cook_county(session, defendant_name, plaintiff_name, case_number, date_from, date_to, limit)
        elif county_key == 'NV_CLARK':
            return await self._search_clark_county(session, defendant_name, plaintiff_name, case_number, date_from, date_to, limit)
        elif county_key == 'GA_FULTON':
            return await self._search_fulton_county(session, defendant_name, plaintiff_name, case_number, date_from, date_to, limit)
        else:
            # Generic web scraper
            return await self._search_generic_court(session, county_key, config, defendant_name, plaintiff_name, address, case_number, limit)
        ...
        for row in rows[1:limit + 1]:
            ...
            if len(cells) >= 4:
                try:
                    ...
                    if not any(prefix in case_number.upper() for prefix in ['CC', 'EV', 'SP']):
                        continue

                    filing_date = self._parse_date(cells[1].get_text(strip=True))
                    parties_text = cells[2].get_text(strip=True) if len(cells) > 2 else ''
    ...
    def _parse_cook_county_json(self, data: Dict[str, Any], limit: int) -> List[EvictionRecord]:
        ...
        for case in cases[:limit]:
            try:
                ...
                record = EvictionRecord(
                    ...
                    parties=[
                        EvictionParty(name=case.get('PlaintiffName', ''), role=PartyRole.PLAINTIFF),
                        EvictionParty(name=case.get('DefendantName', ''), role=PartyRole.DEFENDANT),
                    ],
    ...
    def _parse_clark_county_results(self, html: str, limit: int) -> List[EvictionRecord]:
                ...
                try:
                    ...
                    if filing_date:
                        record = EvictionRecord(
                            ...
                            eviction_type=EvictionType.FORCIBLE_ENTRY_DETAINER,
                            parties=[
                                EvictionParty(name=plaintiff, role=PartyRole.PLAINTIFF) if plaintiff else None,
                                EvictionParty(name=defendant, role=PartyRole.DEFENDANT) if defendant else None,
                            ],
    ...
    def _parse_generic_results(self, html: str, county_key: str, limit: int) -> List[EvictionRecord]:
        """Parse generic court search results."""
        records = []
        soup = BeautifulSoup(html, 'html.parser')

        state = county_key.split('_')[0]
        county = county_key.split('_', 1)[1].replace('_', ' ').title() if '_' in county_key else ''
            ...
            if len(cells) >= 2:
                try:
                    ...
                    for text in text_content:
                        ...
                        if re.match(r'^[\dA-Z]{4,}-', text) or re.match(r'^\d{2,4}[A-Z]{2}', text):
                            case_number = text
    ...
    def _classify_eviction_type(self, cause_text: str) -> EvictionType:
        ...
        if 'nonpayment' in cause_lower or 'non-payment' in cause_lower or 'rent' in cause_lower:
    ...
    def get_coverage_stats(self) -> Dict[str, Any]:
        ...
        states_with_protections = [s for s, c in self.state_configs.items() if c.get('tenant_protections')]
...
def search_evictions_by_defendant(
    ...
    async def _search():
        async with EvictionsAPI() as api:
            records = await api.search_by_defendant(state, defendant_name, county, limit=limit)
...
def search_evictions_by_plaintiff(
    ...
    async def _search():
        async with EvictionsAPI() as api:
            records = await api.search_by_plaintiff(state, plaintiff_name, county, limit=limit)
            return [r.to_dict() for r in records]
