...
class FMCSAApiClient:
    ...
    def _parse_operating_status(
        self, status: Optional[str]
    ) -> Optional[OperatingStatus]:
    ...
    async def get_carrier_by_dot(self, dot_number: str) -> Optional[Carrier]:
        ...
        response = await self._make_request(
            "carrier/profile", {"dotNumber": dot_number}
        )
    ...
    def _parse_carrier(self, data: Dict[str, Any]) -> Optional[Carrier]:
        ...
        dot_number = str(
            carrier_data.get("dotNumber", carrier_data.get("dot_number", ""))
        )
        if not dot_number:
            return None
        ...
        operations = []
        op_class = carrier_data.get(
            "operationClassification", carrier_data.get("operation_classification", [])
        )
        ...
        return Carrier(
            ...
            legal_name=carrier_data.get(
                "legalName", carrier_data.get("legal_name", "")
            ),
            dba_name=carrier_data.get("dbaName", carrier_data.get("dba_name")),
            operating_status=self._parse_operating_status(
                carrier_data.get(
                    "operatingStatus", carrier_data.get("operating_status")
                )
            ),
            ...
            safety_rating_date=self._parse_date(
                carrier_data.get(
                    "safetyRatingDate", carrier_data.get("safety_rating_date")
                )
            ),
            physical_address=carrier_data.get(
                "phyStreet", carrier_data.get("physical_address")
            ),
            physical_city=carrier_data.get(
                "phyCity", carrier_data.get("physical_city")
            ),
            physical_state=carrier_data.get(
                "phyState", carrier_data.get("physical_state")
            ),
            physical_zip=carrier_data.get(
                "phyZipcode", carrier_data.get("physical_zip")
            ),
            mailing_address=carrier_data.get(
                "mailingStreet", carrier_data.get("mailing_address")
            ),
            mailing_city=carrier_data.get(
                "mailingCity", carrier_data.get("mailing_city")
            ),
            mailing_state=carrier_data.get(
                "mailingState", carrier_data.get("mailing_state")
            ),
            mailing_zip=carrier_data.get(
                "mailingZipcode", carrier_data.get("mailing_zip")
            ),
            ...
            power_units=int(
                carrier_data.get("totalPowerUnits", carrier_data.get("power_units", 0))
                or 0
            ),
            drivers=int(
                carrier_data.get("totalDrivers", carrier_data.get("drivers", 0)) or 0
            ),
            operation_classification=operations,
            cargo_carried=carrier_data.get(
                "cargoCarried", carrier_data.get("cargo_carried", [])
            ),
            hazmat=carrier_data.get("carrierHm", carrier_data.get("hazmat")) == "Y",
            passenger_carrier=carrier_data.get(
                "carrierPC", carrier_data.get("passenger_carrier")
            )
            == "Y",
            out_of_service=carrier_data.get(
                "oosFlag", carrier_data.get("out_of_service")
            )
            == "Y",
            oos_date=self._parse_date(
                carrier_data.get("oosDate", carrier_data.get("oos_date"))
            ),
        )
    ...
    async def get_carrier_basics(self, dot_number: str) -> Optional[CarrierBasics]:
        ...
        """
        response = await self._make_request("carrier/basics", {"dotNumber": dot_number})
    ...
    async def get_carrier_inspections(
        ...
        """
        params = {"dotNumber": dot_number, "size": limit}
    ...
    def _parse_inspection(
        self, data: Dict[str, Any], dot_number: str
    ) -> Optional[Inspection]:
        """Parse inspection data from API response"""
        if not data:
            return None

        inspection_id = data.get(
            "inspectionId", data.get("inspection_id", data.get("reportNumber", ""))
        )
        ...
        return Inspection(
            ...
            inspection_date=self._parse_date(
                data.get("inspectionDate", data.get("inspection_date"))
            ),
            ...
            inspection_level=int(
                data.get("inspectionLevel", data.get("level", 0)) or 0
            ),
            ...
            driver_violations=int(
                data.get("driverViolations", data.get("driver_violations", 0)) or 0
            ),
            vehicle_violations=int(
                data.get("vehicleViolations", data.get("vehicle_violations", 0)) or 0
            ),
            hazmat_violations=int(
                data.get("hazmatViolations", data.get("hazmat_violations", 0)) or 0
            ),
            total_violations=int(
                data.get("totalViolations", data.get("total_violations", 0)) or 0
            ),
        )
    ...
    def _parse_crash(self, data: Dict[str, Any], dot_number: str) -> Optional[Crash]:
        ...
        crash_id = data.get(
            "crashId", data.get("crash_id", data.get("reportNumber", ""))
        )
        ...
        return Crash(
            ...
            hazmat_release=data.get("hazmatReleased", data.get("hazmat_release"))
            == "Y",
    ...
    async def get_carrier_authority(
        self, dot_number: str
    ) -> Optional[OperatingAuthority]:
        """
        ...
        """
        response = await self._make_request(
            "carrier/authority", {"dotNumber": dot_number}
        )
        ...
        return OperatingAuthority(
            ...
            common_authority=data.get("commonAuth", data.get("common_authority"))
            == "A",
            contract_authority=data.get("contractAuth", data.get("contract_authority"))
            == "A",
            broker_authority=data.get("brokerAuth", data.get("broker_authority"))
            == "A",
            application_pending=data.get("applicationPending", False),
            effective_date=self._parse_date(
                data.get("effectiveDate", data.get("effective_date"))
            ),
            insurance_required=data.get(
                "insuranceRequired", data.get("insurance_required")
            ),
            insurance_on_file=data.get(
                "insuranceOnFile", data.get("insurance_on_file")
            ),
    ...
    async def get_carrier_oos_summary(self, dot_number: str) -> Dict[str, Any]:
        ...
        """
        response = await self._make_request("carrier/oos", {"dotNumber": dot_number})
    ...
    async def get_carriers_by_state(
        ...
        Returns:
            List of carriers
        """
        params = {"state": state.upper(), "size": limit}
...
def get_carrier_sync(
    dot_number: str, webkey: Optional[str] = None
) -> Optional[Carrier]:
...
def search_carriers_sync(
    ...
    async def _search():
        client = FMCSAApiClient(webkey=webkey)
        try:
            return await client.search_carriers(name=name, state=state, limit=limit)
        finally:
            await client.close()
...
def get_carrier_inspections_sync(
    dot_number: str, limit: int = 100, webkey: Optional[str] = None
) -> List[Inspection]:
    ...
    async def main():
        ...
        try:
            ...
            for carrier in carriers:
                print(
                    f"  DOT {carrier.dot_number}: {carrier.legal_name} ({carrier.physical_state})"
                )
            ...
            if carriers:
                ...
                for insp in inspections:
                    print(
                        f"  {insp.inspection_date}: {insp.total_violations} violations"
                    )
                ...
                for crash in crashes:
                    print(
                        f"  {crash.crash_date}: {crash.fatalities} fatalities, {crash.injuries} injuries"
                    )
