...
from datagod.scrapers.base_api_integration import BaseAPIIntegration, APIKeyAuthentication
...
class CaliforniaCountyAPI(BaseAPIIntegration):
    ...
    COUNTY_APIS = {
        'los-angeles': {
            ...
            'features': ['property_search', 'deed_records', 'mortgage_records', 'tax_info', 'assessor_maps']
        },
        'san-diego': {
    ...
    def __init__(self, jurisdiction_id: int, config: Dict[str, Any]):
        ...
        if self.county_config:
            ...
            self.base_url = f"https://www.{self.county_name.lower()}county.ca.gov/api/assessor"
            self.recorder_url = ''
    ...
    def _search_property_records(self, query: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            properties = data.get('parcels', data.get('properties', data.get('results', [])))
    ...
    def _map_deed_to_standard(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'amount': float(data.get('consideration', data.get('transfer_tax', 0)) * 1000 / 1.1 if data.get('transfer_tax') else 0),
...
class CaliforniaSecretaryOfStateAPI(BaseAPIIntegration, APIKeyAuthentication):
    ...
    def _map_business_to_standard_format(self, api_data: Dict[str, Any]) -> Dict[str, Any]:
