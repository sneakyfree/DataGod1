...
class MortgageDataDataset(Dataset):
    ...
    def __init__(self, records: List[Record], entities: List[Entity], relationships: List[Relationship]):
    ...
    def __getitem__(self, idx):
        ...
        features = torch.tensor([
            ...
            record.date.timestamp() if record.date else 0.0
        ], dtype=torch.float32)
...
class MortgageDataProcessor:
    ...
    def prepare_data(self, records: List[Record], entities: List[Entity], relationships: List[Relationship]) -> Tuple[DataLoader, DataLoader]:
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader, epochs: int = 10):
        ...
        for epoch in range(epochs):
            ...
            logger.info(f'Epoch [{epoch+1}/{epochs}], Train Loss: {running_loss/len(train_loader):.4f}, '
