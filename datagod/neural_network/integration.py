...
from datagod.models import Base, Record, Entity, Relationship
...
class NeuralNetworkIntegration:
    ...
    def initialize_processor(self, input_size: int = 2, hidden_size: int = 128, num_classes: int = 2):
        """Initialize the neural network processor"""
        self.data_processor = MortgageDataProcessor(input_size, hidden_size, num_classes)
    ...
    def gather_mortgage_data(self, jurisdiction_name: str) -> int:
        ...
        try:
            ...
            jurisdiction = self.db_session.query(Jurisdiction).filter_by(name=jurisdiction_name).first()
            if not jurisdiction:
            ...
            logger.info(f"Successfully gathered {len(records)} records for {jurisdiction_name}")
            return len(records)
            
        except Exception as e:
            logger.error(f"Error gathering mortgage data for {jurisdiction_name}: {str(e)}")
    ...
    def process_mortgage_data(self, jurisdiction_name: str):
        ...
        if not self.data_processor:
            logger.warning("Data processor not initialized. Initializing with default parameters.")
        ...
        try:
            ...
            records = self.db_session.query(Record).filter_by(jurisdiction_id=jurisdiction.id).all()
            ...
            train_loader, val_loader = self.data_processor.prepare_data(records, entities, relationships)
            ...
            logger.error(f"Error processing mortgage data for {jurisdiction_name}: {str(e)}")
