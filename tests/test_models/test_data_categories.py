...
class TestCourtCaseRecord:
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import CourtCaseRecord
        columns = {c.name for c in CourtCaseRecord.__table__.columns}
        optional = {'court_name', 'case_title', 'filing_date', 'disposition_date',
                   'judge_name', 'county', 'state', 'amount_claimed', 'amount_awarded',
                   'parties', 'url', 'document_url', 'raw_data'}
...
class TestBusinessEntityRecord:
    ...
    def test_has_required_columns(self):
        ...
        required = {'id', 'jurisdiction_id', 'entity_id', 'entity_name', 'entity_type', 'state'}
        assert required.issubset(columns)
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import BusinessEntityRecord
        columns = {c.name for c in BusinessEntityRecord.__table__.columns}
        optional = {'status', 'formation_date', 'dissolution_date', 'registered_agent_name',
                   'registered_agent_address', 'principal_address', 'officers', 'previous_names',
                   'ein', 'raw_data'}
        assert optional.issubset(columns)
    ...
    def test_repr(self):
        """Test __repr__ method"""
        from datagod.models.data_categories import BusinessEntityRecord
        entity = BusinessEntityRecord(entity_name='Acme Corp', entity_type='corporation')
...
class TestUCCFilingRecord:
    ...
    def test_has_required_columns(self):
        ...
        required = {'id', 'jurisdiction_id', 'filing_number', 'filing_type', 'filing_date'}
        assert required.issubset(columns)
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import UCCFilingRecord
        columns = {c.name for c in UCCFilingRecord.__table__.columns}
        optional = {'lapse_date', 'secured_party', 'secured_party_address', 'debtor_name',
                   'debtor_address', 'collateral_description', 'state', 'amendments', 'raw_data'}
        assert optional.issubset(columns)
...
class TestProfessionalLicenseRecord:
    ...
    def test_has_required_columns(self):
        ...
        required = {'id', 'jurisdiction_id', 'license_number', 'license_type', 'licensee_name', 'state'}
        assert required.issubset(columns)
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import ProfessionalLicenseRecord
        columns = {c.name for c in ProfessionalLicenseRecord.__table__.columns}
        optional = {'licensee_type', 'first_name', 'last_name', 'status', 'issue_date',
                   'expiration_date', 'city', 'address', 'employer_name', 'specializations',
                   'disciplinary_actions', 'nmls_id', 'raw_data'}
        assert optional.issubset(columns)

    def test_repr(self):
        """Test __repr__ method"""
        from datagod.models.data_categories import ProfessionalLicenseRecord
        license = ProfessionalLicenseRecord(license_number='RE-12345', licensee_name='John Agent')
...
class TestTrademarkRecord:
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import TrademarkRecord
        columns = {c.name for c in TrademarkRecord.__table__.columns}
        optional = {'registration_number', 'mark_text', 'mark_description', 'status',
                   'filing_date', 'registration_date', 'owner_name', 'owner_address',
                   'owner_type', 'attorney_name', 'goods_services', 'international_classes',
                   'design_search_codes', 'raw_data'}
        assert optional.issubset(columns)
...
class TestPatentRecord:
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import PatentRecord
        columns = {c.name for c in PatentRecord.__table__.columns}
        optional = {'application_number', 'title', 'abstract', 'patent_type', 'status',
                   'filing_date', 'issue_date', 'expiration_date', 'inventors',
                   'assignee_name', 'assignee_address', 'claims_count', 'classification_codes',
                   'citations', 'raw_data'}
        assert optional.issubset(columns)
...
class TestSECFilingRecord:
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import SECFilingRecord
        columns = {c.name for c in SECFilingRecord.__table__.columns}
        optional = {'company_name', 'ticker', 'accepted_datetime', 'period_of_report',
                   'document_url', 'description', 'file_size', 'sic_code', 'sic_description',
                   'state', 'raw_data'}
        assert optional.issubset(columns)
    ...
    def test_repr(self):
        """Test __repr__ method"""
        from datagod.models.data_categories import SECFilingRecord
        filing = SECFilingRecord(accession_number='0001234567-23-012345', form_type='10-K')
        repr_str = repr(filing)
...
class TestBankRecord:
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import BankRecord
        columns = {c.name for c in BankRecord.__table__.columns}
        optional = {'status', 'charter_type', 'headquarters_city', 'headquarters_state',
                   'headquarters_address', 'established_date', 'insured_date', 'total_assets',
                   'total_deposits', 'branches_count', 'holding_company', 'primary_regulator',
                   'raw_data'}
        assert optional.issubset(columns)
...
class TestNewsArticleRecord:
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import NewsArticleRecord
        columns = {c.name for c in NewsArticleRecord.__table__.columns}
        optional = {'url', 'author', 'description', 'content', 'source_name', 'source_type',
                   'state', 'city', 'category', 'sentiment', 'keywords', 'entities_mentioned',
                   'locations_mentioned', 'image_url', 'raw_data'}
        assert optional.issubset(columns)
...
class TestPressReleaseRecord:
    ...
    def test_has_optional_columns(self):
        ...
        from datagod.models.data_categories import PressReleaseRecord
        columns = {c.name for c in PressReleaseRecord.__table__.columns}
        optional = {'url', 'summary', 'full_text', 'contact_name', 'contact_email',
                   'contact_phone', 'ticker_symbols', 'topics', 'wire_service', 'raw_data'}
        assert optional.issubset(columns)
...
class TestForeignKeys:
    ...
    def test_professional_license_foreign_keys(self):
        ...
        from datagod.models.data_categories import ProfessionalLicenseRecord
        fks = {fk.target_fullname for fk in ProfessionalLicenseRecord.__table__.foreign_keys}
        assert 'jurisdictions.id' in fks
        assert 'data_sources.id' in fks
