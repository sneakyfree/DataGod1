    ...
    def test_researcher_init_creates_directory(self):
        ...
        with patch("os.makedirs") as mock_makedirs:
            ...
            mock_makedirs.assert_called_once_with(
                "datagod/scrapers/data", exist_ok=True
            )
        ...
        with patch("os.makedirs"):
            with patch("builtins.input", return_value="1"):
                with patch("builtins.print") as mock_print:
                    with tempfile.TemporaryDirectory() as tmpdir:
                        with patch.object(
                            JurisdictionResearcher, "__init__", lambda self: None
                        ):
                            researcher = JurisdictionResearcher()
                            researcher.base_dir = tmpdir
                            ...
                            with patch(
                                "datagod.scrapers.jurisdiction_research.JurisdictionResearcher"
                            ) as MockResearcher:
        ...
        with patch("builtins.input", return_value="3"):
            with patch("builtins.print") as mock_print:
                with patch(
                    ...
                    exit_called = any(
                        "Exiting" in str(call) for call in mock_print.call_args_list
                    )
                    assert exit_called
        ...
        with patch("builtins.input", return_value="invalid"):
            with patch("builtins.print") as mock_print:
                with patch(
                    ...
                    invalid_called = any(
                        "Invalid" in str(call) for call in mock_print.call_args_list
                    )
...
class TestJurisdictionDataclassEdgeCases:
    ...
    def test_jurisdiction_with_special_characters_in_name(self):
        ...
        j = Jurisdiction(name="St. Louis County, MO", state="MO", county="St. Louis")
    ...
    def test_jurisdiction_with_apostrophe(self):
        ...
        j = Jurisdiction(
            name="Prince George's County, MD", state="MD", county="Prince George's"
        )
        assert "'" in j.name

    def test_jurisdiction_with_hyphen(self):
        """Test jurisdiction with hyphen in name"""
        j = Jurisdiction(name="Miami-Dade County, FL", state="FL", county="Miami-Dade")
        assert "-" in j.name
