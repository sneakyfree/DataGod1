...
class TestStateAPIInitialization:
    ...
    def test_api_initialization(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_api_has_required_methods(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIAuthentication:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_authenticate_method_exists_and_callable(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIDataMapping:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_map_api_data_with_amount(
        self, mock_session_class, module_name, class_name, base_config
    ):
        ...
        for amount_value in ["$250,000.00", "250000", "250000.00", 250000]:
            api_data = {"id": "REC-001", "amount": amount_value}
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_map_api_data_with_dates(
        self, mock_session_class, module_name, class_name, base_config
    ):
        ...
        for date_format in ["2024-01-15", "01/15/2024", "20240115", "15-01-2024"]:
            api_data = {"id": "REC-001", "record_date": date_format}
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_map_api_data_with_all_fields(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIMetrics:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_get_supported_record_types(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPISearchWithMockedAuth:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_search_records_with_mocked_auth(
        self, mock_session_class, module_name, class_name, base_config
    ):
        """Test search with authentication mocked to always succeed"""
        mock_session = MagicMock()
        mock_response = create_mock_response(
            200, {"results": [{"id": "1", "name": "Test"}]}
        )
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_get_record_details_with_mocked_auth(
        self, mock_session_class, module_name, class_name, base_config
    ):
        """Test get record details with authentication mocked"""
        mock_session = MagicMock()
        mock_response = create_mock_response(
            200, {"id": "1", "document_number": "DOC-001"}
        )
        mock_session.request.return_value = mock_response
        mock_session_class.return_value = mock_session
...
class TestStateAPISpecificSearchMethods:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_search_property_method(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIRateLimiting:
    ...
    @pytest.mark.parametrize(
        "module_name,class_name", STATE_APIS[:10]
    )  # Test first 10 for rate limiting
    @patch("requests.Session")
    ...
    @patch("requests.Session")
    def test_check_rate_limit_method(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIErrorRecovery:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS[:10])
    @patch("requests.Session")
    def test_search_with_no_county_configured(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIIntegration:
    ...
    @pytest.mark.parametrize(
        "module_name,class_name", STATE_APIS[:5]
    )  # Test first 5 for integration
    @patch("requests.Session")
    def test_full_workflow(
        ...
        mock_response = create_mock_response(
            ...
            {
                "results": [
                    {"id": "1", "document_number": "DOC-001", "amount": "$100,000"}
                ]
            },
        )
        mock_session.request.return_value = mock_response
...
class TestStateAPISearchWithCounty:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_search_with_county_set(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_property_search_with_results(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_deed_search_with_results(
        self, mock_session_class, module_name, class_name, base_config
    ):
        ...
        mock_response = create_mock_response(
            200,
            {
                ...
                "deeds": [{"id": "DEED-002", "grantor": "Jane Doe"}],
            },
        )
        mock_session.request.return_value = mock_response
        mock_session_class.return_value = mock_session
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_lien_search_with_results(
        self, mock_session_class, module_name, class_name, base_config
    ):
        ...
        mock_response = create_mock_response(
            200,
            {
                ...
                "liens": [{"id": "LIEN-002", "debtor_name": "Jane Doe"}],
            },
        )
        mock_session.request.return_value = mock_response
        mock_session_class.return_value = mock_session
...
class TestStateAPIAlternateSearchMethods:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_search_with_county_parameter(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIRecordDetails:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_get_record_details_with_county(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_get_record_details_no_county(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIDataMappingAdvanced:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_map_data_with_alternative_field_names(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_map_data_with_party_variants(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_map_data_with_address_variants(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_map_data_with_parcel_variants(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPISearchRecordTypes:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_search_all_record_types(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @patch("requests.Session")
    def test_search_specific_record_type(
        self, mock_session_class, module_name, class_name, record_type, base_config
    ):
...
class TestStateAPISearchParameters:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_search_with_parcel_id(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_search_with_document_number(
        self, mock_session_class, module_name, class_name, base_config
    ):
...
class TestStateAPIStateInfo:
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_get_state_info_complete(
        self, mock_session_class, module_name, class_name, base_config
    ):
    ...
    @pytest.mark.parametrize("module_name,class_name", STATE_APIS)
    @patch("requests.Session")
    def test_get_supported_record_types_returns_list(
        self, mock_session_class, module_name, class_name, base_config
    ):
