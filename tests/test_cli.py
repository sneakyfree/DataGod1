...
class TestRunScraper:
    ...
    def test_run_scraper_specific_jurisdiction(self):
        ...
        with patch("db_manager.DatabaseManager") as mock_db_class:
            mock_db = MagicMock()
            mock_db.get_jurisdiction.return_value = {
                "id": 1,
                "name": "Test County",
                "state": "TX",
            }
...
class TestArgumentParsing:
    ...
    def test_serve_parser_options(self):
        ...
        with patch(
            "sys.argv", ["cli.py", "serve", "--host", "127.0.0.1", "--port", "9000"]
        ):
    ...
    def test_search_parser_options(self):
        ...
        with patch(
            "sys.argv",
            ["cli.py", "search", "mortgage", "--type", "deed", "--limit", "10"],
        ):
    ...
    def test_export_parser_options(self):
        ...
        with patch(
            "sys.argv", ["cli.py", "export", "-o", "output.json", "--format", "csv"]
        ):
